...
Environment.prototype.initialize = function () {
  ...
  this.fillQueueIndex = 0, this.fillFromQueue = this._fillFromQueue.bind(this), this._propChanceModifier = this.propChanceModifier, this._cellSize = this.cellSize, this.areaColors = [[new Float32Array([0, 0.596078431372549, 0.054901960784313725]), new Float32Array([0.1450980392156863, 0.9372549019607843, 0.10980392156862745]), new Float32Array([0.1803921568627451, 0.45098039215686275, 0.23921568627450981])], [new Float32Array([0.8274509803921568, 0.984313725490196, 0.22745098039215686]), new Float32Array([0.9450980392156862, 0.9450980392156862, 0.21176470588235294]), new Float32Array([0.9529411764705882, 0.8274509803921568, 0.45098039215686275])], [new Float32Array([1, 1, 1]), new Float32Array([0.7568627450980392, 0.9098039215686274, 0.9568627450980393]), new Float32Array([0.9372549019607843, 0.9058823529411765, 0.9725490196078431])]], this.tilePools = [], this.tileLocators = [];
  for (var t = this.templateGround.getChildren(), e = 0; e < t.length; e++) {
    ...
    for (var s = 0; 30 > s; s++) {
      var a = o.alloc();
      this.app.root.addChild(a), a.enabled = true, o.free(a);
    }
  }
  ...
  this.tempSphere = {center: new pc.Vec3, radius: 0}, this.currentArea = 0, this.app.on("app:reset", this.reset, this), this.app.on("app:postReset", this.postReset, this), this.app.on("player:current", this.onPlayerProgress, this), this.app.on("level:addBlock", this.onRoadBlockAdded, this), this.app.on("level:removeBlock", this.onRoadBlockRemoved, this);
  ...
  this.fillVisibleCells(true);
}, Environment.prototype.onFreePoolItem = function (e) {
  if (FAST_HIDE) e.model.model.meshInstances[0]._hidden = true; else {
    e.enabled = false;
    var t = e.getParent();
    t && t.removeChild(e);
  }
  ...
  var t = e.getLocalPosition(), i = this.cellX(t.x), n = this.cellZ(t.z), r = this.indexHash(i, n);
  this.getCellPosition(i, n, Environment.v1);
  var o = Environment.v1.x, s = Environment.v1.z;
  t.x >= o ? t.z >= s ? this.topRightRoads[r] = true : this.topLeftRoads[r] = true : t.z >= s ? this.bottomRightRoads[r] = true : this.bottomLeftRoads[r] = true;
  var a = this.props[r];
  if (a) for (var l = 0; l < a.length; l++) a[l].enabled && this.isPositionOnRoad(a[l].getLocalPosition()) && (FAST_HIDE ? a[l].model.model.meshInstances[0]._hidden = true : a[l].enabled = false);
}, Environment.prototype.onRoadBlockRemoved = function (e, t) {
  var i = e.getLocalPosition(), n = this.cellX(i.x), r = this.cellZ(i.z), o = this.indexHash(n, r);
  ...
  for (var i = 1, n = t.length, r = this.propPools.length, o = this.player.getLocalPosition(), s = 0; n > s; s++) {
    ...
    if (!(a > i)) {
      ...
      var h = 0;
      if (!(l.x >= o.x + this._cellSize && l.z >= o.z - this._cellSize && (h = this.treesFinalIndex + 1, l.z > o.z && a > 0.1 * i))) {
        var c = Math.floor(pc.math.random(h, r)), u = this.propPools[c].alloc(), p = this.isPositionOnRoad(l);
        if (FAST_HIDE ? (null === u.getParent() && (this.app.root.addChild(u), u.enabled = true), u.model.model.meshInstances[0]._hidden = p) : (this.app.root.addChild(u), u.enabled = !p), u.setLocalPosition(l), STATIC_MESHES) {
          var f = u.model.model.meshInstances[0];
          u.syncHierarchy(), f._updateAabb = true, f._aabb = f.aabb, f._updateAabb = false;
        }
  ...
  var t = this.cellX(e.x), i = this.cellZ(e.z), n = this.indexHash(t, i);
  this.getCellPosition(t, i, Environment.v2);
  var r = Environment.v2.x, o = Environment.v2.z;
  ...
  var e = this.player.getLocalPosition(), t = this.cellX(e.x), i = this.cellZ(e.z);
  for (var n in this.cellBlocks) {
    var r = this.cellBlocks[n], o = r.getLocalPosition(), s = this.cellX(o.x), a = this.cellZ(o.z);
  ...
  return this.cellBlocks[o] ? false : (Math.abs(e - i) >= 2 || Math.abs(t - n) >= 2) && !this.isCellVisible(i, n) ? false : (!r && this.fillQueueIndex < this.maxFillQueue - 1 ? (this.fillQueue[this.fillQueueIndex].x = i, this.fillQueue[this.fillQueueIndex].z = n, this.fillQueue[this.fillQueueIndex].hash = o, this.fillQueueIndex++) : this._fill(o, i, n), true);
}, Environment.prototype._fill = function (e, t, i) {
  var n = Math.floor(pc.math.random(0, this.tilePools.length));
  cell = this.tilePools[n].alloc(), FAST_HIDE ? null === cell.getParent() && (this.app.root.addChild(cell), cell.enabled = true) : (this.app.root.addChild(cell), cell.enabled = true);
  ...
  if (this.getCellPosition(t, i, r), cell.setPosition(r), STATIC_MESHES) {
    ...
    o && (cell.syncHierarchy(), o._updateAabb = true, o._aabb = o.aabb, o._updateAabb = false), o._hidden = false;
  }
  var s = this.areaColors[this.currentArea], a = s[Math.floor(pc.math.random(0, s.length))];
...
Camera.attributes.add("target", {type: "entity"}), Camera.attributes.add("offset", {type: "vec3", default: [-17, 25, 17]}), Camera.attributes.add("crashOffset", {type: "vec3", default: [-17, 20, 17]}), Camera.attributes.add("crashOffsetPortrait", {type: "vec3", default: [-17, 20, 17]}), Camera.attributes.add("crashZoom", {type: "number", default: 10}), Camera.vecA = new pc.Vec3, Camera.vecB = new pc.Vec3, Camera.extend({initialize: function () {
  this.crashed = false, this.initialZoom = this.entity.camera.orthoHeight, this.initialPosition = Camera.vecA.copy(this.target.getPosition()).add(this.offset).clone(), this.entity.setPosition(this.initialPosition), this.app.renderer.updateCameraFrustum(this.entity.camera.camera), this.app.on("app:reset", this.reset, this), this.app.on("player:crash", this.onCrash, this);
}, reset: function () {
  this.crashed = false, this.entity.setPosition(this.initialPosition), this.entity.camera.orthoHeight = this.initialZoom, this.postUpdate(1e3), this.app.renderer.updateCameraFrustum(this.entity.camera.camera);
}, postUpdate: function (e) {
  var t = this.target, i = this.offset;
  this.crashed && (t = this.target.getParent().script.vehicle.current, i = this.crashOffset, this.app.graphicsDevice.height > 480 && this.app.graphicsDevice.height < 736 && (i = this.crashOffsetPortrait)), t && t.enabled && (Camera.vecA.copy(t.getPosition()).add(i), Camera.vecB.copy(this.entity.getPosition()), Camera.vecB.lerp(Camera.vecB, Camera.vecA, Math.min(1, 0.15 * e / 0.016666666666666666)), this.entity.setPosition(Camera.vecB));
  var n = this.initialZoom;
  this.crashed && (n = this.crashZoom), this.entity.camera.orthoHeight !== n && (this.entity.camera.orthoHeight += (n - this.entity.camera.orthoHeight) * (0.15 * e / 0.016666666666666666), Math.abs(this.entity.camera.orthoHeight - n) < 0.001 && (this.entity.camera.orthoHeight = n));
}, onCrash: function () {
  this.crashed = true;
}, swap: function (e) {
...
Application.attributes.add("frontendstate", {type: "entity"}), Application.attributes.add("ingamestate", {type: "entity"}), Application.attributes.add("gameoverstate", {type: "entity"}), Application.prototype.initialize = function () {
  this.app.on("app:reset", this.reset, this), this.app.gamepads = null, this.initializeStates(), this._dt = [0], this._dt.length = 60, this._index = 0, this._time = 0, this._mode = 0, -1 !== window.location.search.indexOf("autoplay=true") && (this.app.autoplay = true);
...
Share.attributes.add("player", {type: "entity"}), Share.prototype.initialize = function () {
  var e = this;
  ...
  }, false), this.app.ui.shareTwitter.addEventListener("click", function (e) {
    return e.preventDefault(), e.stopPropagation(), false;
  }), this.app.ui.shareTwitter.addEventListener("mousedown", function (t) {
    return t.stopPropagation(), t.preventDefault(), e.share("twitter", true), false;
  }, false)), this.app.ui.shareFacebook && (this.app.ui.shareFacebook.addEventListener("touchstart", function (t) {
    t.stopPropagation(), this.href = e.share("facebook");
  }, false), this.app.ui.shareFacebook.addEventListener("click", function (e) {
    return e.preventDefault(), e.stopPropagation(), false;
  }), this.app.ui.shareFacebook.addEventListener("mousedown", function (t) {
    return t.stopPropagation(), t.preventDefault(), e.share("facebook", true), false;
  }, false));
}, Share.prototype.share = function (e, t) {
  var i = screen.width / 2 - 320, n = screen.height / 2 - 190, r = this.entity.script.score.score, o = this.player.script.player.speed, s = encodeURIComponent("http://swerve.playcanvas.com/"), a = encodeURIComponent("I've scored " + r.toLocaleString() + " speeding " + o + " km/h in SWERVE! Can you beat me? http://swerve.playcanvas.com/ Powered by PlayCanvas #WebGL");
  switch (e) {
    case "facebook":
...
Vehicle.attributes.add("trails", {type: "entity"}), Vehicle.attributes.add("smoke", {type: "entity"}), Vehicle.attributes.add("texture", {type: "asset", assetType: "texture"}), Vehicle.attributes.add("color", {type: "rgb", default: [0, 0.5, 1]}), Vehicle.attributes.add("glossiness", {type: "number", default: 0.5, min: 0, max: 1}), Vehicle.attributes.add("metalness", {type: "number", default: 0, min: 0, max: 1}), Vehicle.vecA = new pc.Vec3, Vehicle.vecB = new pc.Vec3, Vehicle.vecC = new pc.Vec3, Vehicle.vecD = new pc.Vec3, Vehicle.vecE = new pc.Vec3, Vehicle.color = new pc.Color(1, 1, 1), Vehicle.prototype.initialize = function () {
  this.first = true, this.ind = 0, this.current = null, this.trailChildren = this.trails.getChildren().slice(0);
  ...
  var t = this;
  this.image = new Image, this.imageLoaded = false, this.image.onload = function () {
    t.imageLoaded = true, t.renderTexture();
  ...
  return 0 > i && (i += 1), i > 1 && (i -= 1), 0.16666666666666666 > i ? e + 6 * (t - e) * i : 0.5 > i ? t : 0.6666666666666666 > i ? e + (t - e) * (0.6666666666666666 - i) * 6 : e;
}, Vehicle.prototype.colorHslToRgb = function (e, t, i) {
  var n, r, o;
  if (0 == t) n = r = o = i; else {
    var s = 0.5 > i ? i * (1 + t) : i + t - i * t, a = 2 * i - s;
    n = this._hue2rgb(a, s, e + 0.3333333333333333), r = this._hue2rgb(a, s, e), o = this._hue2rgb(a, s, e - 0.3333333333333333);
  }
  return [n, r, o];
}, Vehicle.prototype.update = function (e) {
  this.first && (this.first = false, this.reset()), this.current && (this.current.pipe || (this.app.vehicles.process(this.current), this.attachTrails()));
  ...
  var e = this.colorHslToRgb(Math.random(), 1, 0.5);
  ...
  var r = this.colorHslToRgb(Math.random(), 1, 0.5);
  this.color.set(r[0], r[1], r[2]), this.renderTexture();
}, Vehicle.prototype.set = function (e) {
  this.current !== e && (this.current && (this.entity.removeChild(this.current), this.current.enabled = false), this.current = e, pc.GraphNode.prototype.addChild.call(this.entity, this.current), this.current.setLocalPosition(0, 0, 0), this.current.enabled = true, this.attachTrails());
...
Score.attributes.add("levelDistance", {type: "number", default: 10}), Score.attributes.add("bonusMultiplier", {type: "number", default: 5, placeholder: "x"}), Score.prototype.initialize = function () {
...
var Player = new pc.Script("player"), count = 0;
Player.attributes.add("speed", {type: "number", default: 50, placeholder: "km/h"}), Player.attributes.add("acceleration", {type: "number", default: 5, placeholder: "km/h"}), Player.attributes.add("turnSpeed", {type: "number", default: 90, placeholder: "deg/s"}), Player.left = 180, Player.right = 90, Player.vecA = new pc.Vec3, Player.vecB = new pc.Vec3, Player.vecC = new pc.Vec3, Player.size = 1, Player.timeEasing = 0.05, Player.extend({initialize: function () {
  this.started = false, this.introduced = 0, this.left = true, this.angle = new pc.Angle(Player.left), this.angleTarget = new pc.Angle(Player.left), this.angleRoll = new pc.Angle, this.angleOffN = new pc.Angle, this.angleA = this.angle.clone(), this.block = 0, this.crashed = false, this.timeScale = 1, this.lastTimeScale = 1, this.crashRotate = new pc.Vec3, this.crashY = 0, this.startTime = Date.now(), this.startSpeed = this.speed, this.blockLast = null, this.garagePos = new pc.Vec3(-100, 0, -100), this.immediateStop = false;
  var e = this;
  ...
  }, window.addEventListener("mousedown", this.onClick, false), window.addEventListener("touchstart", this.onClick, false), this.app.on("app:start", this.onStart, this), this.app.on("app:reset", this.onReset, this), this.app.on("player:level", this.onLevel, this), this.app.on("garage:spawn", this.onGarageSpawn, this), this.app.on("ui:template", this.onTemplate, this), this.onTemplate(), this.app.on("audio:mute", function (e) {
  ...
  this.lastTimeScale = this.timeScale, !this.crashed && 2 !== this.introduced || 0 === this.timeScale ? !this.crashed && 2 !== this.introduced && this.started && 1 !== this.timeScale && (this.timeScale += (1 - this.timeScale) * Math.min(1, Player.timeEasing * e / 0.016666666666666666), this.timeScale > 0.999 && (this.timeScale = 1)) : (this.timeScale += (0 - this.timeScale) * Math.min(1, Player.timeEasing * e / 0.016666666666666666), this.timeScale < 0.001 && (this.timeScale = 0));
}, updateSound: function () {
  this.entity.sound.pitch = this.timeScale, this.app.root._children[0].sound.pitch = 0.5 + 0.5 * this.timeScale, this.app.root._children[0].sound.volume = 0.1 + 0.3 * this.timeScale;
  ...
  0 !== t ? this.angleOffN.degrees += 45 * e * this.timeScale * (t > 0 ? 1 : -1) : this.angleOffN.degrees += (0 - this.angleOffN.degrees) * (0.2 * e / 0.016666666666666666 * this.timeScale), this.angleA.degrees = this.angle.degrees + this.angleOffN.degrees;
  var i = Math.min(1, Math.abs(this.angleA.distance(this.angleTarget)));
  this.app.mute || (this.entity.sound.slot("skidding").volume = 0.5 * i);
  for (var n = this.entity.script.vehicle.trailChildren, r = 0; r < n.length; r++) {
    var o = 1;
    if (this.blockLast) {
      if (n[r].pivot) {
        var s = this.app.level.getBlock(this.block), a = this.app.level.getBlockType(this.block), l = s.getLocalPosition(), h = n[r].pivot.getPosition();
        switch (a) {
    ...
    n[r].level = Math.min(1, 0.2 + i + (this.crashed ? 0 : 2 * (1 - this.timeScale)) + this.crashed) * o;
  }
  ...
  var t = this.app.level.getBlock(this.block), i = this.app.level.getBlockType(this.block), n = this.entity.getLocalPosition(), r = t.getLocalPosition();
  switch (this.blockLast = t, i) {
    case BLOCK_STRAIGHT_LEFT:
      (n.x > r.x + 4 - Player.size || n.x < r.x - 4 + Player.size) && (this.crashed = true);
      break;
    case BLOCK_STRAIGHT_RIGHT:
      (n.z > r.z + 4 - Player.size || n.z < r.z - 4 + Player.size) && (this.crashed = true);
      break;
    case BLOCK_CORNER_LEFT:
      (n.z > r.z + 4 - Player.size || n.x > r.x + 4 - Player.size) && (this.crashed = true);
      break;
    case BLOCK_CORNER_RIGHT:
      (n.z < r.z - 4 + Player.size || n.x < r.x - 4 + Player.size) && (this.crashed = true);
  }
  this.crashed ? (this.crashRotate.x = 30 * (2 * Math.random() - 1), this.crashRotate.y = 15 * (2 * Math.random() - 1), this.crashRotate.z = 5 * (2 * Math.random() - 1), this.crashY = 0.3 * Math.random(), this.app.mute || this.entity.sound.slot("crash").play(), this.app.fire("player:crash"), this.app.ui.element && this.app.ui.element.classList.add("gameOver"), this.app.ui["game-over.speed"] && (this.app.ui["game-over.speed"].textContent = this.speed), this.garagePos && Math.abs(this.garagePos.x - n.x) < 4 + Player.size && Math.abs(this.garagePos.z - n.z) < 4 + Player.size ? this.immediateStop = true : this.immediateStop = false, 3 !== this.introduced && (this.introduced = 0, this.app.ui.introduce && this.app.ui.introduce.classList.remove("active"))) : n.x > r.x + 4 || n.z < r.z - 4 || n.x < r.x - 4 || n.z > r.z + 4 ? (this.block++, this.app.fire("player:current", this.block), 0 === this.introduced && 2 === this.block && (this.introduced = 1)) : 1 === this.introduced && n.z - 1 < r.z && (this.introduced = this.app.autoplay ? 3 : 2, !this.app.autoplay && this.app.ui.introduce && this.app.ui.introduce.classList.add("active"));
  ...
  if (this.app.ui.restart) {
    ...
    this.app.ui.restart.addEventListener("touchstart", t, false), this.app.ui.restart.addEventListener("click", t, false);
  }
}, onStart: function () {
  this.started = true;
}, onReset: function () {
  this.entity.setPosition(0, 1.15, 0), this.angle.degrees = Player.left, this.angleTarget.degrees = this.angle.degrees, this.angleRoll.degrees = 0, this.angleOffN.degrees = 0, this.left = true, this.block = 0, this.crashed = false, this.timeScale = 0, this.startTime = Date.now(), this.speed = this.startSpeed, this.blockLast = null, this.app.ui.speed && (this.app.ui.speed.textContent = this.speed), this.app.ui.element && this.app.ui.element.classList.remove("gameOver"), this.app.mute || this.entity.sound.slot("start").play(), this.entity.sound.slot("crash").stop();
}, swap: function (e) {
  ...
  }, window.addEventListener("mousedown", this.onClick, false), window.addEventListener("touchstart", this.onClick, false), this.app.off("app:start", e.onStart, e), this.app.on("app:start", this.onStart, this), this.app.off("app:reset", e.onReset, e), this.app.on("app:reset", this.onReset, this), this.app.off("player:level", e.onLevel, e), this.app.on("player:level", this.onLevel, this), this.app.off("garage:spawn", e.onGarageSpawn, e), this.app.on("garage:spawn", this.onGarageSpawn, this), this.app.off("ui:template", e.onTemplate, e), this.app.on("ui:template", this.onTemplate, this);
}});
var Collectibles = new pc.Script("collectibles");
Collectibles.attributes.add("templatePoints", {type: "entity"}), Collectibles.attributes.add("iconSpeed", {type: "number", default: 2}), Collectibles.attributes.add("iconFadeHeight", {type: "number", default: 5}), Collectibles.attributes.add("iconFadeSpeed", {type: "number", default: 2}), Collectibles.attributes.add("player", {type: "entity"}), Collectibles.attributes.add("camera", {type: "entity"}), Collectibles.attributes.add("chance", {type: "vec2", default: [5, 15]}), Collectibles.attributes.add("confetti", {type: "entity"}), Collectibles.v1 = new pc.Vec2, Collectibles.v2 = new pc.Vec2, Collectibles.v3 = new pc.Vec3, Collectibles.sphere = {center: new pc.Vec3, radius: 2}, Collectibles.prototype.initialize = function () {
  ...
  if (this.blocksTillCollectible--, !(this.blocksTillCollectible > 0)) {
    ...
    i.setLocalScale(0.85, 0.85, 0.85), i.setLocalEulerAngles(0, 360 * Math.random(), 0), this.app.root.addChild(i);
    var n = Collectibles.v3;
    n.copy(e.getLocalPosition()), n.y += 2, t === BLOCK_STRAIGHT_LEFT ? n.x += pc.math.random(0, 1) < 0.5 ? 2 : -2 : t === BLOCK_STRAIGHT_RIGHT ? n.z += pc.math.random(0, 1) < 0.5 ? 2 : -2 : t === BLOCK_CORNER_LEFT ? pc.math.random(0, 1) < 0.5 ? (n.x -= 2, n.z += pc.math.random(0, 1) < 0.5 ? -2 : 2) : (n.z -= 2, n.x += pc.math.random(0, 1) < 0.5 ? -2 : 2) : t === BLOCK_CORNER_RIGHT && (pc.math.random(0, 1) < 0.5 ? (n.x += 2, n.z += pc.math.random(0, 1) < 0.5 ? -2 : 2) : (n.z += 2, n.x += pc.math.random(0, 1) < 0.5 ? -2 : 2)), i.setPosition(n), i.enabled = true, this.collectibles[e.getGuid()] = i;
  }
  ...
  for (var i in this.collectibles) if (this.icons[i]) {
    var n = this.collectibles[i], r = false;
    if (Collectibles.sphere.center.copy(n.getLocalPosition()), this.camera.camera.camera.getFrustum().containsSphere(Collectibles.sphere)) {
    ...
    t = s.getLocalPosition(), Collectibles.v1.set(t.x, t.z), Collectibles.v1.sub(Collectibles.v2), Collectibles.v1.lengthSq() < 4 && (this.app.fire("collectible:points"), this.icons[i] = true, this.app.mute || s.sound.slot("hit").play());
  }
};
var Music = new pc.Script("music");
Music.prototype.initialize = function () {
  if (this.app.mute = false, this.entity.sound.slot("start").play().on("end", this.onStartEnd, this), this.app.ui.mute) {
    var e = this, t = function (t) {
    ...
    this.app.ui.mute.addEventListener("mousedown", t, false), this.app.ui.mute.addEventListener("touchstart", t, false);
  }
...
Powered.prototype.initialize = function () {
  var e = this, t = false, i = null, n = function (t) {
    t.preventDefault(), t.stopPropagation(), i && clearTimeout(i), r(), e.app.fire("tap");
  }, r = function () {
    t || (t = true, e.app.ui.powered && (e.app.ui.powered.removeEventListener("mousedown", n), e.app.ui.powered.removeEventListener("touchstart", n), e.app.ui.powered.classList.add("hidden"), e.app.fire("app:start"), setTimeout(function () {
      e.app.ui.powered && e.app.ui.powered.parentNode.removeChild(e.app.ui.powered);
    }, 1500)));
  };
  e.app.ui.powered && (e.app.ui.powered.addEventListener("mousedown", n, false), e.app.ui.powered.addEventListener("touchstart", n, false)), setTimeout(function () {
    t || (i = setTimeout(r, 1500));
  }, 0);
};
var BLOCK_SIZE = 8, MAX_BLOCKS = 13, TAIL = 5, FIRST_POS = new pc.Vec3, LEAD = MAX_BLOCKS - TAIL - 1, BLOCK_STRAIGHT_LEFT = 1, BLOCK_STRAIGHT_RIGHT = 2, BLOCK_CORNER_LEFT = 3, BLOCK_CORNER_RIGHT = 4, Level = new pc.Script("level");
Level.attributes.add("roadStraight", {type: "entity"}), Level.attributes.add("roadRight", {type: "entity"}), Level.attributes.add("roadLeft", {type: "entity"}), Level.tmpVec = new pc.Vec3, Level.v2 = new pc.Vec3, Level.FAST_HIDE = true, Level.prototype.initialize = function () {
  ...
  for (var e = 0; 12 > e; e++) {
    ...
    this.app.root.addChild(t), t.enabled = true, this.poolStraight.free(t);
  }
  this.poolLeft = new EntityPool(this.roadLeft, 12), this.poolLeft.on("free", this.onFreePoolItem, this);
  for (var e = 0; 12 > e; e++) {
    var t = this.poolLeft.alloc();
    this.app.root.addChild(t), t.enabled = true, this.poolLeft.free(t);
  }
  ...
  for (var e = 0; 12 > e; e++) {
    ...
    this.app.root.addChild(t), t.enabled = true, this.poolRight.free(t);
  }
  this.blocks = [], this.app.on("player:current", this.onPlayerProgress, this), this.app.on("app:collect", this.onCollect, this), this.app.on("app:reset", this.reset, this), this.app.level = this, this.progress = 0, this.forceStraight = false;
}, Level.prototype.postInitialize = function () {
  this.reset();
}, Level.prototype.reset = function () {
  this.poolStraight.freeAll(), this.poolRight.freeAll(), this.poolLeft.freeAll(), this.blocks = new Array(MAX_BLOCKS), this.progress = 0, this.forceStraight = false;
  var e = 3;
  this.forceStraight = true;
  ...
  if ("s" === t.name[0]) {
    ...
    return i.y < 0.1 ? BLOCK_STRAIGHT_RIGHT : BLOCK_STRAIGHT_LEFT;
  }
  return "l" === t.name[0] ? BLOCK_CORNER_LEFT : BLOCK_CORNER_RIGHT;
}, Level.prototype.addBlock = function (e) {
  var t = e - 1;
  0 > t && (t += MAX_BLOCKS);
  var i = Level.tmpVec, n = BLOCK_STRAIGHT_LEFT, r = this.getBlockType(t);
  if (r) {
    if (i.copy(this.blocks[t].getLocalPosition()), r === BLOCK_STRAIGHT_LEFT || r === BLOCK_CORNER_LEFT) if (i.z -= BLOCK_SIZE, this.forceStraight) n = BLOCK_STRAIGHT_LEFT; else {
      for (var o = true, s = false, a = 1; 4 >= a; a++) {
        var l = t - a, h = this.getBlockType(l);
        if (!h) break;
        h === BLOCK_STRAIGHT_LEFT || h === BLOCK_STRAIGHT_RIGHT ? s = true : o = false;
      }
      n = o ? BLOCK_CORNER_RIGHT : s ? pc.math.random(0, 1) < 0.5 ? BLOCK_STRAIGHT_LEFT : BLOCK_CORNER_RIGHT : BLOCK_STRAIGHT_LEFT;
    } else if (r === BLOCK_STRAIGHT_RIGHT || r === BLOCK_CORNER_RIGHT) if (i.x += BLOCK_SIZE, this.forceStraight) n = BLOCK_STRAIGHT_RIGHT; else {
      for (var o = true, s = false, a = 1; 4 >= a; a++) {
        var l = t - a, h = this.getBlockType(l);
        if (!h) break;
        h === BLOCK_STRAIGHT_RIGHT || h === BLOCK_STRAIGHT_LEFT ? s = true : o = false;
      }
      n = o ? BLOCK_CORNER_LEFT : s ? pc.math.random(0, 1) < 0.5 ? BLOCK_STRAIGHT_RIGHT : BLOCK_CORNER_LEFT : BLOCK_STRAIGHT_RIGHT;
    }
  ...
  var c, u = Level.v2;
  if (n === BLOCK_STRAIGHT_LEFT ? (c = this.poolStraight.alloc(), u.set(0, 90, 0)) : n === BLOCK_STRAIGHT_RIGHT ? (c = this.poolStraight.alloc(), u.set(0, 0, 0)) : n === BLOCK_CORNER_LEFT ? (c = this.poolLeft.alloc(), u.set(0, 0, 0)) : (c = this.poolRight.alloc(), u.set(0, 90, 0)), Level.FAST_HIDE ? (null === c.getParent() && (this.app.root.addChild(c), c.enabled = true), c.model.model.meshInstances[0]._hidden = false, c.model.model.meshInstances[1]._hidden = false) : (this.app.root.addChild(c), c.enabled = true), c.setLocalEulerAngles(u), c.setPosition(i), STATIC_MESHES) {
    c.syncHierarchy();
    for (var p = c.model.model.meshInstances, a = 0; a < p.length; a++) p[a]._updateAabb = true, p[a]._aabb = p[a].aabb, p[a]._updateAabb = false;
  }
  ...
  if (Level.FAST_HIDE) e.model.model.meshInstances[0]._hidden = true, e.model.model.meshInstances[1]._hidden = true; else {
    e.enabled = false;
    var t = e.getParent();
    t && t.removeChild(e);
  }
...
Ui.attributes.add("html", {type: "asset", assetType: "html"}), Ui.attributes.add("less", {type: "asset", assetType: "css"}), Ui.prototype.initialize = function () {
  this.app.ui = {};
  ...
  e.innerHTML = this.replaceTags(t.resource), this.app.ui = {element: e};
...
Vehicles.attributes.add("entities", {type: "entity"}), Vehicles.prototype.postInitialize = function () {
  this.list = [];
...
CameraFrustum.attributes.add("camera", {type: "entity"}), CameraFrustum.vecA = new pc.Vec3, CameraFrustum.vecB = new pc.Vec3, CameraFrustum.color = new pc.Color(1, 1, 1), CameraFrustum.prototype.postUpdate = function (e) {
...
Fps.prototype.initialize = function () {
  this.meter = new FPSMeter({graph: true, top: "auto", bottom: "50px"});
  ...
  return function e(t, i, n) {
    ...
    var i = e("./utils").addDataAttr, n = e("./browser");
    t.exports = function (e, t) {
      i(t, n.currentScript(e)), undefined === t.isFileProtocol && (t.isFileProtocol = /^(file|(chrome|safari)(-extension)?|resource|qrc|app):/.test(e.location.protocol)), t.async = t.async || false, t.fileAsync = t.fileAsync || false, t.poll = t.poll || (t.isFileProtocol ? 1e3 : 1500), t.env = t.env || ("127.0.0.1" == e.location.hostname || "0.0.0.0" == e.location.hostname || "localhost" == e.location.hostname || e.location.port && e.location.port.length > 0 || t.isFileProtocol ? "development" : "production");
      var r = /!dumpLineNumbers:(comments|mediaquery|all)/.exec(e.location.hash);
      r && (t.dumpLineNumbers = r[1]), undefined === t.useFileCache && (t.useFileCache = true), undefined === t.onReady && (t.onReady = true);
    };
  }, {"./browser": 3, "./utils": 9}], 2: [function (e, t) {
  ...
  }, {"./add-default-options": 1, "./index": 7, "promise/polyfill.js": 95}], 3: [function (e, t) {
    var i = e("./utils");
    t.exports = {createCSS: function (e, t, n) {
      var r = n.href || "", o = "less:" + (n.title || i.extractId(r)), s = e.getElementById(o), a = false, l = e.createElement("style");
      ...
      if (s && a === false && s.parentNode.removeChild(s), l.styleSheet) try {
    ...
    t.exports = function (e, t, i) {
      ...
      return {setCSS: function (e, t, r) {
        ...
        var i = n && n.getItem(e), r = n && n.getItem(e + ":timestamp");
        return r && t.lastModified && new Date(t.lastModified).valueOf() === new Date(r).valueOf() ? i : undefined;
      }};
    };
  }, {}], 5: [function (e, t) {
    var i = e("./utils"), n = e("./browser");
    t.exports = function (e, t, r) {
      function o(t, o) {
        var s, a, l = "less-error-message:" + i.extractId(o || ""), h = '<li><label>{line}</label><pre class="{class}">{content}</pre></li>', c = e.document.createElement("div"), u = [], p = t.filename || o, f = p.match(/([^\/]+(\?.*)?)$/)[1];
        ...
        var d = function (e, t, i) {
          undefined !== e.extract[t] && u.push(h.replace(/\{line\}/, (parseInt(e.line, 10) || 0) + (t - 1)).replace(/\{class\}/, i).replace(/\{content\}/, e.extract[t]));
        };
        t.extract && (d(t, 0, ""), d(t, 1, "line"), d(t, 2, ""), a += "on line " + t.line + ", column " + (t.column + 1) + ":</p><ul>" + u.join("") + "</ul>"), t.stack && (t.extract || r.logLevel >= 4) && (a += "<br/>Stack Trace</br />" + t.stack.split("\n").slice(1).join("<br/>")), c.innerHTML = a, n.createCSS(e.document, [".less-error-message ul, .less-error-message li {", "list-style-type: none;", "margin-right: 15px;", "padding: 4px 0;", "margin: 0;", "}", ".less-error-message label {", "font-size: 12px;", "margin-right: 15px;", "padding: 4px 0;", "color: #cc7777;", "}", ".less-error-message pre {", "color: #dd6666;", "padding: 4px 0;", "margin: 0;", "display: inline-block;", "}", ".less-error-message pre.line {", "color: #ff0000;", "}", ".less-error-message h3 {", "font-size: 20px;", "font-weight: bold;", "padding: 15px 0 5px 0;", "margin: 0;", "}", ".less-error-message a {", "color: #10a", "}", ".less-error-message .error {", "color: red;", "font-weight: bold;", "padding-bottom: 2px;", "border-bottom: 1px dashed red;", "}"].join("\n"), {title: "error-message"}), c.style.cssText = ["font-family: Arial, sans-serif", "border: 1px solid #e00", "background-color: #eee", "border-radius: 5px", "-webkit-border-radius: 5px", "-moz-border-radius: 5px", "color: #e00", "padding: 15px", "margin-bottom: 15px"].join(";"), "development" === r.env && (s = setInterval(function () {
          var t = e.document, i = t.body;
      ...
      function c(e, i) {
        var n = "{line} {content}", o = e.filename || i, s = [], a = (e.type || "Syntax") + "Error: " + (e.message || "There is an error in your .less file") + " in " + o + " ", l = function (e, t, i) {
          undefined !== e.extract[t] && s.push(n.replace(/\{line\}/, (parseInt(e.line, 10) || 0) + (t - 1)).replace(/\{class\}/, i).replace(/\{content\}/, e.extract[t]));
        };
      ...
      return {add: s, remove: h};
    };
  }, {"./browser": 3, "./utils": 9}], 6: [function (e, t) {
    t.exports = function (t, i) {
      ...
      var r = e("../less/environment/abstract-file-manager.js"), o = {}, s = function () {};
      return s.prototype = new r, s.prototype.alwaysMakePathsAbsolute = function () {
        ...
        var l = n(), h = t.isFileProtocol ? t.fileAsync : t.async;
        ...
        if (i.useFileCache && o[a]) try {
          var l = o[a];
          r(null, {contents: l, filename: a, webInfo: {lastModified: new Date}});
        } catch (h) {
          r({filename: a, message: "Error loading file " + a + " error was " + h.message});
        } else this.doXHR(a, i.mime, function (e, t) {
          o[a] = e, r(null, {contents: e, filename: a, webInfo: {lastModified: t}});
        }, function (e, t) {
          r({type: "File", message: "'" + t + "' wasn't found (" + e + ")", href: a});
        });
      }, s;
    };
  }, {"../less/environment/abstract-file-manager.js": 14}], 7: [function (e, t) {
    var i = e("./utils").addDataAttr, n = e("./browser");
    t.exports = function (t, r) {
      ...
      function h(e, t, n, a, l) {
        function h(i) {
          var r = i.contents, s = i.filename, l = i.webInfo, h = {currentDirectory: v.getPath(s), filename: s, rootFilename: s, relativeUrls: c.relativeUrls};
          if (h.entryPath = h.currentDirectory, h.rootpath = c.rootpath || h.currentDirectory, l && (l.remaining = a, !c.modifyVars)) {
            var u = y.getCSS(s, l);
            if (!n && u) return l.local = true, void t(null, u, r, e, l, s);
          }
          g.remove(s), c.rootFileInfo = h, f.render(r, c, function (i, n) {
            i ? (i.href = s, t(i)) : (n.css = (r.postProcessor && "function" == typeof r.postProcessor && (n.css = r.postProcessor.call(n.css, n.css) || n.css), n.css), c.modifyVars || y.setCSS(e.href, l.lastModified, n.css), t(null, n.css, r, e, l, s));
          });
        }
        var c = s(r);
      ...
      var p = t.document, f = e("../less")();
      f.options = r;
      var d = f.environment, m = e("./file-manager")(r, f.logger), v = new m;
      d.addFileManager(v), f.FileManager = m, e("./log-listener")(f, r);
      var g = e("./error-reporting")(t, f, r), y = f.cache = r.cache || e("./cache")(t, r, f.logger);
      r.functions && f.functions.functionRegistry.addMultiple(r.functions);
      var b = /^text\/(x-)?less$/;
      return f.watch = function () {
        return f.watchMode || (f.env = "development", u()), this.watchMode = true, true;
      }, f.unwatch = function () {
        return clearInterval(f.watchTimer), this.watchMode = false, false;
        ...
        return f.refresh(true, e, false);
      }, f.refresh = function (e, i, r) {
        return (e || r) && r !== false && v.clearFileCache(), new Promise(function (r, o) {
          var s, a, h;
          s = a = new Date, c(function (e, i, l, c, u) {
            return e ? (g.add(e, e.href || c.href), void (r.postProcessor && "function" == typeof r.postProcessor && (e = r.postProcessor.call(e, e) || e), e)) : (f.logger.info(u.local ? "loading " + c.href + " from cache." : "rendered " + c.href + " successfully."), n.createCSS(t.document, i, c), f.logger.info("css for " + c.href + " generated in " + (new Date - a) + "ms"), 0 === u.remaining && (h = new Date - s, f.logger.info("less has finished. css generated in " + h + "ms"), r({startTime: s, endTime: a, totalMilliseconds: h, sheets: f.sheets.length})), void (a = new Date));
          }, e, i), l(i);
        });
      }, f.refreshStyles = l, f;
    };
  }, {"../less": 30, "./browser": 3, "./cache": 4, "./error-reporting": 5, "./file-manager": 6, "./log-listener": 8, "./utils": 9}], 8: [function (e, t) {
    t.exports = function (e, t) {
      var i = 4, n = 3, r = 2, o = 1;
      t.logLevel = "undefined" != typeof t.logLevel ? t.logLevel : "development" === t.env ? n : o, t.loggers || (t.loggers = [{debug: function (e) {
    ...
    t.exports = {extractId: function (e) {
  ...
  }, {"./colors": 11, "./unit-conversions": 13}], 13: [function (e, t) {
    t.exports = {length: {m: 1, cm: 0.01, mm: 0.001, in: 0.0254, px: 0.0002645833333333333, pt: 0.00035277777777777776, pc: 0.004233333333333333}, duration: {s: 1, ms: 0.001}, angle: {rad: 1 / (2 * Math.PI), deg: 0.002777777777777778, grad: 0.0025, turn: 1}};
  }, {}], 14: [function (e, t) {
    var i = function () {};
    i.prototype.getPath = function (e) {
      ...
      var i, n, r, o, s = this.extractUrlParts(e), a = this.extractUrlParts(t), l = "";
      ...
      var i, n, r = /^((?:[a-z-]+:)?\/+?(?:[^\/\?#]*\/)|([\/\\]))?((?:[^\/\\\?#]*[\/\\])*)([^\/\\\?#]*)([#\?].*)?$/i, o = e.match(r), s = {}, a = [];
  ...
  }, {"../logger": 32}], 16: [function (e) {
    function t(e, t, n) {
      var r, o, s, a, l = t.alpha, h = n.alpha, c = [];
      s = h + l * (1 - h);
      for (var u = 0; 3 > u; u++) r = t.rgb[u] / 255, o = n.rgb[u] / 255, a = e(r, o), s && (a = (h * o + l * (r - h * (r + o - a))) / s), c[u] = 255 * a;
      return new i(c, s);
    }
    var i = e("../tree/color"), n = e("./function-registry"), r = {multiply: function (e, t) {
      return e * t;
    }, screen: function (e, t) {
      return e + t - e * t;
      ...
      var i = 1, n = e;
      return t > 0.5 && (n = 1, i = e > 0.25 ? Math.sqrt(e) : ((16 * e - 12) * e + 4) * e), e - (1 - 2 * t) * n * (i - e);
    ...
    n.addMultiple(t);
  }, {"../tree/color": 49, "./function-registry": 21}], 17: [function (e) {
    function n(e) {
    ...
    var o, s = e("../tree/dimension"), a = e("../tree/color"), l = e("../tree/quoted"), h = e("../tree/anonymous"), c = e("./function-registry");
    o = {rgb: function (e, t, i) {
      ...
      e = n(e) % 360 / 360, i = Math.min(1, Math.max(0, n(i))), r = Math.min(1, Math.max(0, n(r))), s = Math.min(1, Math.max(0, n(s)));
      var l = 0.5 >= r ? r * (i + 1) : r + i - r * i, h = 2 * r - l;
      return o.rgba(255 * (e + 0.3333333333333333 = 0 > e + 0.3333333333333333 ? e + 0.3333333333333333 + 1 : e + 0.3333333333333333 > 1 ? e + 0.3333333333333333 - 1 : e + 0.3333333333333333, 1 > 6 * (e + 0.3333333333333333) ? h + (l - h) * (e + 0.3333333333333333) * 6 : 1 > 2 * (e + 0.3333333333333333) ? l : 2 > 3 * (e + 0.3333333333333333) ? h + (l - h) * (0.6666666666666666 - (e + 0.3333333333333333)) * 6 : h), 255 * (e = 0 > e ? e + 1 : e > 1 ? e - 1 : e, 1 > 6 * e ? h + (l - h) * e * 6 : 1 > 2 * e ? l : 2 > 3 * e ? h + (l - h) * (0.6666666666666666 - e) * 6 : h), 255 * (e - 0.3333333333333333 = 0 > e - 0.3333333333333333 ? e - 0.3333333333333333 + 1 : e - 0.3333333333333333 > 1 ? e - 0.3333333333333333 - 1 : e - 0.3333333333333333, 1 > 6 * (e - 0.3333333333333333) ? h + (l - h) * (e - 0.3333333333333333) * 6 : 1 > 2 * (e - 0.3333333333333333) ? l : 2 > 3 * (e - 0.3333333333333333) ? h + (l - h) * (0.6666666666666666 - (e - 0.3333333333333333)) * 6 : h), s);
    }, hsv: function (e, t, i) {
      ...
      var l = [i, i * (1 - t), i * (1 - a * t), i * (1 - (1 - a) * t)], h = [[0, 3, 1], [2, 0, 1], [1, 0, 3], [1, 2, 0], [3, 1, 0], [0, 1, 2]];
      ...
      var o = e.toHSL();
      return o.s += "undefined" != typeof r && "relative" === r.value ? o.s * n.value / 100 : n.value / 100, o.s = Math.min(1, Math.max(0, o.s)), o.hsla(o.h, o.s, o.l, o.a);
    }, desaturate: function (e, n, r) {
      var o = e.toHSL();
      return o.s -= "undefined" != typeof r && "relative" === r.value ? o.s * n.value / 100 : n.value / 100, o.s = Math.min(1, Math.max(0, o.s)), o.hsla(o.h, o.s, o.l, o.a);
    }, lighten: function (e, n, r) {
      var o = e.toHSL();
      return o.l += "undefined" != typeof r && "relative" === r.value ? o.l * n.value / 100 : n.value / 100, o.l = Math.min(1, Math.max(0, o.l)), o.hsla(o.h, o.s, o.l, o.a);
    }, darken: function (e, n, r) {
      var o = e.toHSL();
      return o.l -= "undefined" != typeof r && "relative" === r.value ? o.l * n.value / 100 : n.value / 100, o.l = Math.min(1, Math.max(0, o.l)), o.hsla(o.h, o.s, o.l, o.a);
    }, fadein: function (e, n, r) {
      var o = e.toHSL();
      return o.a += "undefined" != typeof r && "relative" === r.value ? o.a * n.value / 100 : n.value / 100, o.a = Math.min(1, Math.max(0, o.a)), o.hsla(o.h, o.s, o.l, o.a);
    }, fadeout: function (e, n, r) {
      var o = e.toHSL();
      return o.a -= "undefined" != typeof r && "relative" === r.value ? o.a * n.value / 100 : n.value / 100, o.a = Math.min(1, Math.max(0, o.a)), o.hsla(o.h, o.s, o.l, o.a);
    }, fade: function (e, n) {
      var r = e.toHSL();
      return r.a = n.value / 100, r.a = Math.min(1, Math.max(0, r.a)), o.hsla(r.h, r.s, r.l, r.a);
    }, spin: function (e, t) {
      var n = e.toHSL(), r = (n.h + t.value) % 360;
      return n.h = 0 > r ? 360 + r : r, o.hsla(n.h, n.s, n.l, n.a);
    }, mix: function (e, t, i) {
      e.toHSL && t.toHSL || (console.log(t.type), console.dir(t)), i || (i = new s(50));
      var n = i.value / 100, r = 2 * n - 1, o = e.toHSL().a - t.toHSL().a, l = ((r * o == -1 ? r : (r + o) / (1 + r * o)) + 1) / 2, h = 1 - l, c = [e.rgb[0] * l + t.rgb[0] * h, e.rgb[1] * l + t.rgb[1] * h, e.rgb[2] * l + t.rgb[2] * h], u = e.alpha * n + t.alpha * (1 - n);
      return new a(c, u);
      ...
      return r = "undefined" == typeof r ? 0.43 : n(r), e.luma() < r ? i : t;
    }, argb: function (e) {
      return new h(e.toARGB());
    }, color: function (e) {
      if (e instanceof l && /^#([a-f0-9]{6}|[a-f0-9]{3})$/i.test(e.value)) return new a(e.value.slice(1));
      if (e instanceof a || (e = a.fromKeyword(e.value))) return e.value = undefined, e;
  ...
  }, {"../tree/anonymous": 45, "../tree/color": 49, "../tree/dimension": 55, "../tree/quoted": 72, "./function-registry": 21}], 18: [function (e, t) {
    t.exports = function (t) {
      var i = e("../tree/quoted"), n = e("../tree/url"), r = e("./function-registry"), s = e("../logger");
      r.add("data-uri", function (e, r) {
        r || (r = e, e = null);
        var a = e && e.value, l = r.value, h = this.currentFileInfo, c = h.relativeUrls ? h.currentDirectory : h.entryPath, u = l.indexOf("#"), p = "";
        -1 !== u && (p = l.slice(u), l = l.slice(0, u));
        var f = t.getFileManager(l, c, this.context, t, true);
        if (!f) return new n(r, this.index, this.currentFileInfo).eval(this.context);
        var d = false;
        if (e) d = /;base64$/.test(a); else {
          if (a = t.mimeLookup(l), "image/svg+xml" === a) d = false; else {
        ...
        if (!v.contents) return s.warn("Skipped data-uri embedding of " + l + " because file not found"), new n(r || e, this.index, this.currentFileInfo).eval(this.context);
        var g = v.contents;
        if (d && !t.encodeBase64) return new n(r, this.index, this.currentFileInfo).eval(this.context);
        g = d ? t.encodeBase64(g) : encodeURIComponent(g);
        var y = "data:" + a + "," + g + p, b = 32768;
        return y.length >= b && this.context.ieCompat !== false ? (s.warn("Skipped data-uri embedding of " + l + " because its size (" + y.length + " characters) exceeds IE8-safe " + b + " characters!"), new n(r || e, this.index, this.currentFileInfo).eval(this.context)) : new n(new i('"' + y + '"', y, false, this.index, this.currentFileInfo), this.index, this.currentFileInfo);
      });
    };
  }, {"../logger": 32, "../tree/quoted": 72, "../tree/url": 79, "./function-registry": 21}], 19: [function (e, t) {
    var i = e("../tree/keyword"), n = e("./function-registry"), r = {eval: function () {
      var e = this.value_, t = this.error_;
      if (t) throw t;
      return null != e ? e ? i.True : i.False : undefined;
    }, value: function (e) {
      this.value_ = e;
    }, error: function (e) {
      this.error_ = e;
  ...
  }, {"../tree/keyword": 64, "./function-registry": 21}], 20: [function (e, t) {
    var i = e("../tree/expression"), n = function (e, t, i, n) {
  ...
  }, {"../tree/expression": 58}], 21: [function (e, t) {
    function i(e) {
      return {_data: {}, add: function (e, t) {
    ...
    t.exports = function (t) {
      var i = {functionRegistry: e("./function-registry"), functionCaller: e("./function-caller")};
      return e("./default"), e("./color"), e("./color-blending"), e("./data-uri")(t), e("./math"), e("./number"), e("./string"), e("./svg")(t), e("./types"), i;
    };
  }, {"./color": 17, "./color-blending": 16, "./data-uri": 18, "./default": 19, "./function-caller": 20, "./function-registry": 21, "./math": 24, "./number": 25, "./string": 26, "./svg": 27, "./types": 28}], 23: [function (e, t) {
    var i = e("../tree/dimension"), n = function () {};
    n._math = function (e, t, n) {
      if (!(n instanceof i)) throw {type: "Argument", message: "argument must be a number"};
      return null == t ? t = n.unit : n = n.unify(), new i(e(parseFloat(n.value)), t);
    }, t.exports = n;
  }, {"../tree/dimension": 55}], 24: [function (e) {
    var t = e("./function-registry"), i = e("./math-helper.js"), n = {ceil: null, floor: null, sqrt: null, abs: null, tan: "", sin: "", cos: "", atan: "rad", asin: "rad", acos: "rad"};
  ...
  }, {"./function-registry": 21, "./math-helper.js": 23}], 25: [function (e) {
    var t = e("../tree/dimension"), i = e("../tree/anonymous"), n = e("./function-registry"), r = e("./math-helper.js"), o = function (e, n) {
      switch (n = Array.prototype.slice.call(n), n.length) {
        case 0:
          throw {type: "Argument", message: "one or more arguments required"};
      }
      var r, o, s, a, l, h, c, u, p = [], f = {};
      for (r = 0; n.length > r; r++) if (s = n[r], s instanceof t) if (a = "" === s.unit.toString() && undefined !== u ? new t(s.value, u).unify() : s.unify(), h = "" === a.unit.toString() && undefined !== c ? c : a.unit.toString(), c = "" !== h && undefined === c || "" !== h && "" === p[0].unify().unit.toString() ? h : c, u = "" !== h && undefined === u ? s.unit.toString() : u, o = undefined !== f[""] && "" !== h && h === c ? f[""] : f[h], undefined !== o) l = "" === p[o].unit.toString() && undefined !== u ? new t(p[o].value, u).unify() : p[o].unify(), (e && l.value > a.value || !e && a.value > l.value) && (p[o] = s); else {
        if (undefined !== c && h !== c) throw {type: "Argument", message: "incompatible types"};
    ...
    n.addMultiple({min: function () {
      return o(true, arguments);
    }, max: function () {
      return o(false, arguments);
      ...
      if ("number" == typeof e && "number" == typeof i) e = new t(e), i = new t(i); else if (!(e instanceof t && i instanceof t)) throw {type: "Argument", message: "arguments must be numbers"};
  ...
  }, {"../tree/anonymous": 45, "../tree/dimension": 55, "./function-registry": 21, "./math-helper.js": 23}], 26: [function (e) {
    var t = e("../tree/quoted"), i = e("../tree/anonymous"), n = e("../tree/javascript"), r = e("./function-registry");
    r.addMultiple({e: function (e) {
  ...
  }, {"../tree/anonymous": 45, "../tree/javascript": 62, "../tree/quoted": 72, "./function-registry": 21}], 27: [function (e, t) {
    t.exports = function () {
      var t = e("../tree/dimension"), i = e("../tree/color"), n = e("../tree/expression"), r = e("../tree/quoted"), o = e("../tree/url"), s = e("./function-registry");
      s.add("svg-gradient", function (e) {
        ...
        var a, l, h, c, u, p, f, d, m = "linear", v = 'x="0" y="0" width="1" height="1"', g = {compress: false}, y = e.toCSS(g);
        ...
        for (h = '<?xml version="1.0" ?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%" viewBox="0 0 1 1" preserveAspectRatio="none"><' + m + 'Gradient id="gradient" gradientUnits="userSpaceOnUse" ' + l + ">", c = 0; a.length > c; c += 1) a[c] instanceof n ? (u = a[c].value[0], p = a[c].value[1]) : (u = a[c], p = undefined), u instanceof i && ((0 === c || c + 1 === a.length) && undefined === p || p instanceof t) || s(), f = p ? p.toCSS(g) : 0 === c ? "0%" : "100%", d = u.alpha, h += '<stop offset="' + f + '" stop-color="' + u.toRGB() + '"' + (1 > d ? ' stop-opacity="' + d + '"' : "") + "/>";
        return h += "</" + m + "Gradient><rect " + v + ' fill="url(#gradient)" /></svg>', h = encodeURIComponent(h), h = "data:image/svg+xml," + h, new o(new r("'" + h + "'", h, false, this.index, this.currentFileInfo), this.index, this.currentFileInfo);
      });
    };
  }, {"../tree/color": 49, "../tree/dimension": 55, "../tree/expression": 58, "../tree/quoted": 72, "../tree/url": 79, "./function-registry": 21}], 28: [function (e) {
    var t = e("../tree/keyword"), i = e("../tree/detached-ruleset"), n = e("../tree/dimension"), r = e("../tree/color"), o = e("../tree/quoted"), s = e("../tree/anonymous"), a = e("../tree/url"), l = e("../tree/operation"), h = e("./function-registry"), c = function (e, i) {
      return e instanceof i ? t.True : t.False;
    }, u = function (e, i) {
      if (undefined === i) throw {type: "Argument", message: "missing the required second argument to isunit."};
      if (i = "string" == typeof i.value ? i.value : i, "string" != typeof i) throw {type: "Argument", message: "Second argument to isunit should be a unit or a string."};
      return e instanceof n && e.unit.is(i) ? t.True : t.False;
    }, p = function (e) {
      var t = Array.isArray(e.value) ? e.value : Array(e);
      return t;
    };
    h.addMultiple({isruleset: function (e) {
      return c(e, i);
    }, iscolor: function (e) {
      return c(e, r);
    }, isnumber: function (e) {
      ...
      return u(e, "em");
    }, isunit: u, unit: function (e, i) {
      if (!(e instanceof n)) throw {type: "Argument", message: "the first argument to unit must be a number" + (e instanceof l ? ". Have you forgotten parenthesis?" : "")};
  ...
  }, {"../tree/anonymous": 45, "../tree/color": 49, "../tree/detached-ruleset": 54, "../tree/dimension": 55, "../tree/keyword": 64, "../tree/operation": 70, "../tree/quoted": 72, "../tree/url": 79, "./function-registry": 21}], 29: [function (e, t) {
    var i = e("./contexts"), n = e("./parser/parser"), r = e("./plugins/function-importer");
    t.exports = function (e) {
      ...
      return t.prototype.push = function (t, o, s, a, l) {
        ...
        var c = function (e, i, n) {
          ...
          a.optional && e ? l(null, {rules: []}, false, null) : (h.files[n] = i, e && !h.error && (h.error = e), l(e, i, r, n));
        }, u = {relativeUrls: this.context.relativeUrls, entryPath: s.entryPath, rootpath: s.rootpath, rootFilename: s.rootFilename}, p = e.getFileManager(t, s.currentDirectory, this.context, e);
        if (!p) return void c({message: "Could not find a file-manager for " + t});
        ...
        var f = function (e) {
          ...
          l.processImports = false, h.contents[t] = o, (s.reference || a.reference) && (u.reference = true), a.plugin ? new r(l, u).eval(o, function (e, i) {
            c(e, i, t);
  ...
  }, {"./contexts": 10, "./parser/parser": 37, "./plugins/function-importer": 39}], 30: [function (e, t) {
    t.exports = function (t, i) {
      var n, r, o, s, a, l = {version: [2, 5, 3], data: e("./data"), tree: e("./tree"), Environment: a = e("./environment/environment"), AbstractFileManager: e("./environment/abstract-file-manager"), environment: t = new a(t, i), visitors: e("./visitors"), Parser: e("./parser/parser"), functions: e("./functions")(t), contexts: e("./contexts"), SourceMapOutput: n = e("./source-map-output")(t), SourceMapBuilder: r = e("./source-map-builder")(n, t), ParseTree: o = e("./parse-tree")(r), ImportManager: s = e("./import-manager")(t), render: e("./render")(t, o, s), parse: e("./parse")(t, o, s), LessError: e("./less-error"), transformTree: e("./transform-tree"), utils: e("./utils"), PluginManager: e("./plugin-manager"), logger: e("./logger")};
      return l;
    };
  }, {"./contexts": 10, "./data": 12, "./environment/abstract-file-manager": 14, "./environment/environment": 15, "./functions": 22, "./import-manager": 29, "./less-error": 31, "./logger": 32, "./parse": 34, "./parse-tree": 33, "./parser/parser": 37, "./plugin-manager": 38, "./render": 40, "./source-map-builder": 41, "./source-map-output": 42, "./transform-tree": 43, "./tree": 61, "./utils": 82, "./visitors": 86}], 31: [function (e, t) {
    var i = e("./utils"), n = t.exports = function (e, t, n) {
      Error.call(this);
      var r = e.filename || n;
      if (t && r) {
        var o = t.contents[r], s = i.getLocation(e.index, o), a = s.line, l = s.column, h = e.call && i.getLocation(e.call, o).line, c = o.split("\n");
    ...
    var i = e("./less-error"), n = e("./transform-tree"), r = e("./logger");
    t.exports = function (e) {
      var t = function (e, t) {
      ...
      return t.prototype.toCSS = function (t) {
        ...
        try {
          ...
          var c = {compress: h, dumpLineNumbers: t.dumpLineNumbers, strictUnits: Boolean(t.strictUnits), numPrecision: 8};
          t.sourceMap ? (s = new e(t.sourceMap), a.css = s.toCSS(o, c, this.imports)) : a.css = o.toCSS(c);
        } catch (l) {
          throw new i(l, this.imports);
        }
        if (t.pluginManager) for (var u = t.pluginManager.getPostProcessors(), p = 0; u.length > p; p++) a.css = u[p].process(a.css, {sourceMap: s, options: t, imports: this.imports});
  ...
  }, {"./less-error": 31, "./logger": 32, "./transform-tree": 43}], 34: [function (e, t) {
    var i, n = e("./contexts"), r = e("./parser/parser"), o = e("./plugin-manager");
    t.exports = function (t, s, a) {
      var l = function (t, s, h) {
        ...
        if (f.addPlugins(s.plugins), s.pluginManager = f, u = new n.Parse(s), s.rootFileInfo) p = s.rootFileInfo; else {
          var d = s.filename || "input", m = d.replace(/[^\/\\]*$/, "");
          p = {filename: d, relativeUrls: u.relativeUrls, rootpath: u.rootpath || "", currentDirectory: m, entryPath: m, rootFilename: d}, p.rootpath && "/" !== p.rootpath.slice(-1) && (p.rootpath += "/");
        }
        var v = new a(u, p);
  ...
  }, {"./contexts": 10, "./parser/parser": 37, "./plugin-manager": 38, promise: undefined}], 35: [function (e, t) {
    t.exports = function (e, t) {
      function i(t) {
        var i = a - v;
        512 > i && !t || !i || (m.push(e.slice(v, a + 1)), v = a + 1);
      }
      var n, r, o, s, a, l, h, c, u, p = e.length, f = 0, d = 0, m = [], v = 0;
      for (a = 0; p > a; a++) if (h = e.charCodeAt(a), !(h >= 97 && 122 >= h || 34 > h)) switch (h) {
        ...
        case 47:
          if (d || a == p - 1) continue;
          if (c = e.charCodeAt(a + 1), 47 == c) for (a += 2; p > a && (c = e.charCodeAt(a), !(13 >= c) || 10 != c && 13 != c); a++) ; else if (42 == c) {
      ...
      return 0 !== f ? o > n && s > o ? t("missing closing `}` or `*/`", n) : t("missing closing `}`", n) : 0 !== d ? t("missing closing `)`", r) : (i(true), m);
    };
  }, {}], 36: [function (e, t) {
    var i = e("./chunker");
    t.exports = function () {
      var e, t, n, r, o, s, a, l = [], h = {};
      h.save = function () {
        a = h.i, l.push({current: s, i: h.i, j: t});
        ...
        var i = h.i + (t || 0), n = e.charCodeAt(i);
      ...
      var c = 32, u = 9, p = 10, f = 13, d = 43, m = 44, v = 47, g = 57;
      h.autoCommentAbsorb = true, h.commentStore = [], h.finished = false;
      var y = function (i) {
        for (var n, r, l, d = h.i, m = t, g = h.i - a, b = h.i + s.length - g, w = h.i += i, x = e; b > h.i; h.i++) {
          if (n = x.charCodeAt(h.i), h.autoCommentAbsorb && n === v) {
            if (r = x.charAt(h.i + 1), "/" === r) {
              l = {index: h.i, isLineComment: true};
              var S = x.indexOf("\n", h.i + 2);
            ...
            if ("*" === r) {
              ...
              if (C >= 0) {
                l = {index: h.i, text: x.substr(h.i, C + 2 - h.i), isLineComment: false}, h.i += l.text.length - 1, h.commentStore.push(l);
                continue;
              }
            }
            break;
        ...
        if (s = s.slice(i + h.i - w + g), a = h.i, !s.length) {
          if (o.length - 1 > t) return s = o[++t], y(0), true;
          h.finished = true;
        }
        return d !== h.i || m !== t;
      };
      return h.peek = function (t) {
        if ("string" == typeof t) {
          for (var i = 0; t.length > i; i++) if (e.charAt(h.i + i) !== t.charAt(i)) return false;
          return true;
        }
        return t.test(s);
        ...
        return n > h.i && (t = r, h.i = n), {isFinished: i, furthest: h.i, furthestPossibleErrorMessage: t, furthestReachedEnd: h.i >= e.length - 1, furthestChar: e[h.i]};
      }, h;
    };
  }, {"./chunker": 35}], 37: [function (e, t) {
    var i = e("../less-error"), n = e("../tree"), r = e("../visitors"), o = e("./parser-input"), s = e("../utils"), a = function l(e, t, a) {
      function h(e, t) {
      ...
      function u(e, n) {
        throw new i({index: d.i, filename: a.filename, type: n || "Syntax", message: e}, t);
      }
      function p(e) {
        var t = a.filename;
      ...
      var f, d = o();
      return {parse: function (o, s, h) {
        var c, u, p, f, m = null, v = "";
        if (u = h && h.globalVars ? l.serializeVars(h.globalVars) + "\n" : "", p = h && h.modifyVars ? "\n" + l.serializeVars(h.modifyVars) : "", e.pluginManager) for (var g = e.pluginManager.getPreProcessors(), y = 0; g.length > y; y++) o = g[y].process(o, {context: e, imports: t, fileInfo: a});
        ...
        try {
          d.start(o, e.chunkInput, function (e, n) {
            throw new i({index: n, type: "Parse", message: e, filename: a.filename}, t);
          }), c = new n.Ruleset(null, this.parsers.primary()), c.root = true, c.firstRoot = true;
        } catch (b) {
        ...
        if (!w.isFinished) {
          ...
          x || (x = "Unrecognised input", "}" === w.furthestChar ? x += ". Possibly missing opening '{'" : ")" === w.furthestChar ? x += ". Possibly missing opening '('" : w.furthestReachedEnd && (x += ". Possibly missing something")), m = new i({type: "Parse", message: x, index: w.furthest, filename: a.filename}, t);
        }
        return e.processImports === false ? (e = m || e || t.error, e ? (e instanceof i || (e = new i(e, t, a.filename)), s(e)) : s(null, c)) : void new r.ImportVisitor(t, S).run(c);
      }, parsers: f = {primary: function () {
        for (var e, t = this.mixin, i = [];;) {
          for (; e = this.comment(), e;) i.push(e);
          if (d.finished) break;
          if (d.peek("}")) break;
          if (e = this.extendRule()) i = i.concat(e); else if (e = t.definition() || this.rule() || this.ruleset() || t.call() || this.rulesetCall() || this.directive()) i.push(e); else {
            for (var n = false; d.$char(";");) n = true;
            if (!n) break;
          }
        }
        return i;
        ...
        var e, t = d.i, i = false;
        return d.save(), d.$char("~") && (i = true), (e = d.$quoted()) ? (d.forget(), new n.Quoted(e.charAt(0), e.substr(1, e.length - 2), i, t, a)) : void d.restore();
      }, keyword: function () {
        var e = d.$char("%") || d.$re(/^[_A-Za-z-][_A-Za-z0-9-]*/);
        return e ? n.Color.fromKeyword(e) || new n.Keyword(e) : undefined;
      }, call: function () {
        var e, t, i, r, o = d.i;
        return d.peek(/^url\(/i) ? undefined : (d.save(), (e = d.$re(/^([\w-]+|%|progid:[\w\.]+)\(/)) ? (e = e[1], t = e.toLowerCase(), "alpha" === t && (r = f.alpha()) ? (d.forget(), r) : (i = this.arguments(), d.$char(")") ? (d.forget(), new n.Call(e, i, o, a)) : void d.restore("Could not parse call arguments or missing ')'"))) : void d.forget());
      }, arguments: function () {
        for (var e, t = []; (e = this.assignment() || f.expression(), e) && (t.push(e), d.$char(","));) ;
        return t;
      }, literal: function () {
        ...
        var e, t = d.i;
        return d.autoCommentAbsorb = false, d.$str("url(") ? (e = this.quoted() || this.variable() || d.$re(/^(?:(?:\\[\(\)'"])|[^\(\)'"])+/) || "", d.autoCommentAbsorb = true, c(")"), new n.URL(null != e.value || e instanceof n.Variable ? e : new n.Anonymous(e), t, a)) : void (d.autoCommentAbsorb = true);
      }, variable: function () {
        var e, t = d.i;
        return "@" === d.currentChar() && (e = d.$re(/^@@?[\w-]+/)) ? new n.Variable(e, t, a) : undefined;
      }, variableCurly: function () {
        var e, t = d.i;
        return "@" === d.currentChar() && (e = d.$re(/^@\{([\w-]+)\}/)) ? new n.Variable("@" + e[1], t, a) : undefined;
      }, color: function () {
        var e;
        if ("#" === d.currentChar() && (e = d.$re(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})/))) {
          var t = e.input.match(/^#([\w]+).*/);
          return t = t[1], t.match(/^[A-Fa-f0-9]+$/) || u("Invalid HEX color code"), new n.Color(e[1], undefined, "#" + t);
        }
      }, dimension: function () {
        if (!d.peekNotNumeric()) {
          var e = d.$re(/^([+-]?\d*\.?\d+)(%|[a-z]+)?/i);
          return e ? new n.Dimension(e[1], e[2]) : undefined;
        }
      }, unicodeDescriptor: function () {
        var e;
        return e = d.$re(/^U\+[0-9a-fA-F?]+(\-[0-9a-fA-F?]+)?/), e ? new n.UnicodeDescriptor(e[0]) : undefined;
      }, javascript: function () {
        var e, t = d.i;
        d.save();
        var i = d.$char("~"), r = d.$char("`");
        ...
        var e;
        return "@" === d.currentChar() && (e = d.$re(/^(@[\w-]+)\s*:/)) ? e[1] : undefined;
      }, rulesetCall: function () {
        var e;
        return "@" === d.currentChar() && (e = d.$re(/^(@[\w-]+)\s*\(\s*\)\s*;/)) ? new n.RulesetCall(e[1]) : undefined;
      }, extend: function (e) {
        var t, i, r, o, s, a = d.i;
        ...
        var e, t, i, r, o, s, l = d.currentChar(), h = false, u = d.i;
        if ("." === l || "#" === l) {
          for (d.save(); e = d.i, r = d.$re(/^[#.](?:[\w-]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+/), r;) i = new n.Element(o, r, e, a), t ? t.push(i) : t = [i], o = d.$char(">");
          return t && (d.$char("(") && (s = this.args(true).args, c(")")), f.important() && (h = true), f.end()) ? (d.forget(), new n.mixin.Call(t, s, u, a, h)) : void d.restore();
        }
      }, args: function (e) {
        var t, i, r, o, s, a, l, h = f.entities, c = {args: null, variadic: false}, p = [], m = [], v = [];
        for (d.save();;) {
          if (e) a = f.detachedRuleset() || f.expression(); else {
            if (d.commentStore.length = 0, d.$str("...")) {
              c.variadic = true, d.$char(";") && !t && (t = true), (t ? m : v).push({variadic: true});
              break;
            }
          ...
          if (e ? a.value && 1 == a.value.length && (g = a.value[0]) : g = a, g && g instanceof n.Variable) if (d.$char(":")) {
            if (p.length > 0 && (t && u("Cannot mix ; and , as delimiter types"), i = true), s = f.detachedRuleset() || f.expression(), !s) {
            ...
            if (!e) {
              c.variadic = true, d.$char(";") && !t && (t = true), (t ? m : v).push({name: a.name, variadic: true});
              break;
            }
            l = true;
          } else e || (r = o = g.name, s = null);
          s && p.push(s), v.push({name: o, value: s, expand: l}), d.$char(",") || (d.$char(";") || t) && (i && u("Cannot mix ; and , as delimiter types"), t = true, p.length > 1 && (s = new n.Value(p)), m.push({name: r, value: s, expand: l}), r = null, p = [], i = false);
        }
        return d.forget(), c.args = t ? m : v, c;
      }, definition: function () {
        var e, t, i, r, o = [], s = false;
        if (!("." !== d.currentChar() && "#" !== d.currentChar() || d.peek(/^[^{]*\}/))) if (d.save(), t = d.$re(/^([#.](?:[\w-]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+)\s*\(/)) {
          e = t[1];
          var a = this.args(false);
        ...
        var e;
        return d.$re(/^opacity=/i) ? (e = d.$re(/^\d+/), e || (e = h(this.entities.variable, "Could not parse alpha")), c(")"), new n.Alpha(e)) : undefined;
      }, element: function () {
        var e, t, i, r = d.i;
        return t = this.combinator(), e = d.$re(/^(?:\d+\.\d+|\d+)%/) || d.$re(/^(?:[.#]?|:*)(?:[\w-]|[^\x00-\x9f]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+/) || d.$char("*") || d.$char("&") || this.attribute() || d.$re(/^\([^&()@]+\)/) || d.$re(/^[\.#:](?=@)/) || this.entities.variableCurly(), e || (d.save(), d.$char("(") ? (i = this.selector()) && d.$char(")") ? (e = new n.Paren(i), d.forget()) : d.restore("Missing closing ')'") : d.forget()), e ? new n.Element(t, e, r, a) : undefined;
      }, combinator: function () {
        var e = d.currentChar();
        if ("/" === e) {
          d.save();
        ...
        for (var t, i, r, o, s, l, c, p = d.i; (e && (i = this.extend()) || e && (l = d.$str("when")) || (o = this.element())) && (l ? c = h(this.conditions, "expected condition") : c ? u("CSS guard can only be used at the end of selector") : i ? s = s ? s.concat(i) : i : (s && u("Extend can only be used at the end of selector"), r = d.currentChar(), t ? t.push(o) : t = [o], o = null), "{" !== r && "}" !== r && ";" !== r && "," !== r && ")" !== r);) ;
        ...
        var e;
        return d.$char("{") && (e = this.primary()) && d.$char("}") ? e : undefined;
      }, blockRuleset: function () {
        var e = this.block();
        return e && (e = new n.Ruleset(null, e)), e;
      }, detachedRuleset: function () {
        var e = this.blockRuleset();
        return e ? new n.DetachedRuleset(e) : undefined;
      }, ruleset: function () {
        var t, i, r, o;
        for (d.save(), e.dumpLineNumbers && (o = p(d.i)); (i = this.lessSelector(), i) && (t ? t.push(i) : t = [i], d.commentStore.length = 0, i.condition && t.length > 1 && u("Guards are only currently allowed on a single selector."), d.$char(","));) i.condition && u("Guards are only currently allowed on a single selector."), d.commentStore.length = 0;
        if (t && (r = this.block())) {
          d.forget();
        ...
        var i, r, o, s, l, h = d.i, c = d.currentChar();
        if ("." !== c && "#" !== c && "&" !== c && ":" !== c) if (d.save(), i = this.variable() || this.ruleProperty()) {
          if (l = "string" == typeof i, l && (r = this.detachedRuleset()), d.commentStore.length = 0, !r) {
            ...
            if (u && (r = this.value()), !r && (r = this.anonymousValue())) return d.forget(), new n.Rule(i, r, false, s, h, a);
            u || r || (r = this.value()), o = this.important();
          }
          if (r && this.end()) return d.forget(), new n.Rule(i, r, o, s, h, a);
          if (d.restore(), r && !t) return this.rule(true);
        } else d.forget();
      }, anonymousValue: function () {
        var e = d.$re(/^([^@+\/'"*`(;{}-]*);/);
        return e ? new n.Anonymous(e[1]) : undefined;
      }, import: function () {
        var e, t, i = d.i, r = d.$re(/^@import?\s+/);
        if (r) {
        ...
        d.save();
        do if (e = i.keyword() || i.variable()) r.push(e); else if (d.$char("(")) {
          if (t = this.property(), e = this.value(), !d.$char(")")) return d.restore("Missing closing ')'"), null;
          if (t && e) r.push(new n.Paren(new n.Rule(t, e, null, null, d.i, a, true))); else {
            if (!e) return d.restore("badly formed media feature definition"), null;
            r.push(new n.Paren(e));
          }
        } while (e);
        return d.forget(), r.length > 0 ? new n.Expression(r) : undefined;
        ...
        var t, i, r, o, s, l, h, c = d.i, f = true, m = true;
        if ("@" === d.currentChar()) {
        ...
        if (e = this.operand()) {
          for (o = d.isWhitespace(-1); !d.peek(/^\/[*\/]/);) {
            ...
            d.forget(), e.parensInOp = true, t.parensInOp = true, r = new n.Operation(i, [r || e, t], o), o = d.isWhitespace(-1);
          }
          return r || e;
        }
      }, addition: function () {
        ...
        if (e = this.multiplication()) {
          for (o = d.isWhitespace(-1); (i = d.$re(/^[-+]\s+/) || !o && (d.$char("+") || d.$char("-")), i) && (t = this.multiplication(), t);) e.parensInOp = true, t.parensInOp = true, r = new n.Operation(i, [r || e, t], o), o = d.isWhitespace(-1);
          return r || e;
        }
      }, conditions: function () {
        var e, t, i, r = d.i;
        ...
        function e(e) {
          var t = d.i, i = d.$re(e);
          return i ? (o.push(t), r.push(i[1])) : undefined;
        }
        var t, i, r = [], o = [];
        d.save();
  ...
  }, {"../less-error": 31, "../tree": 61, "../utils": 82, "../visitors": 86, "./parser-input": 36}], 38: [function (e, t) {
    var i = function (e) {
    ...
    var i = e("../less-error"), n = e("../tree"), r = t.exports = function (e, t) {
      this.fileInfo = t;
    };
    r.prototype.eval = function (e, t) {
      ...
      try {
        ...
        t(new i({message: "Plugin evaluation error: '" + a.name + ": " + a.message.replace(/["]/g, "'") + "'", filename: this.fileInfo.filename}), null);
      }
      t(null, {functions: s});
    };
  }, {"../less-error": 31, "../tree": 61}], 40: [function (e, t) {
    var i;
    ...
    t.exports = function (e, t) {
      ...
      return i.prototype.toCSS = function (t, i, n) {
        ...
        if (this.options.sourceMapFileInline) {
          if (undefined === this.sourceMap) return "";
    ...
    t.exports = function (e) {
      ...
      return t.prototype.normalizeFilename = function (e) {
        ...
        if (this._sourceMapGenerator = new this._sourceMapGeneratorConstructor({file: this._outputFilename, sourceRoot: null}), this._outputSourceFiles) for (var t in this._contentsMap) if (this._contentsMap.hasOwnProperty(t)) {
    ...
    var i = e("./contexts"), n = e("./visitors"), r = e("./tree");
    t.exports = function (e, t) {
      t = t || {};
      var o, s = t.variables, a = new i.Eval(t);
      "object" != typeof s || Array.isArray(s) || (s = Object.keys(s).map(function (e) {
        var t = s[e];
        return t instanceof r.Value || (t instanceof r.Expression || (t = new r.Expression([t])), t = new r.Value([t])), new r.Rule("@" + e, t, false, null, 0);
      }), a.frames = [new r.Ruleset(null, s)]);
      var l, h = [], c = [new n.JoinSelectorVisitor, new n.ExtendVisitor, new n.ToCSSVisitor({compress: Boolean(t.compress)})];
      if (t.pluginManager) {
  ...
  }, {"./contexts": 10, "./tree": 61, "./visitors": 86}], 44: [function (e, t) {
  ...
  }, {"./node": 69}], 45: [function (e, t) {
    var i = e("./node"), n = function (e, t, i, n, r, o) {
      this.value = e, this.index = t, this.mapLines = n, this.currentFileInfo = i, this.rulesetLike = "undefined" == typeof r ? false : r, this.isReferenced = o || false;
    };
    n.prototype = new i, n.prototype.type = "Anonymous", n.prototype.eval = function () {
      ...
      return e.toCSS && this.toCSS() === e.toCSS() ? 0 : undefined;
  ...
  }, {"./node": 69}], 46: [function (e, t) {
    var i = e("./node"), n = function (e, t) {
  ...
  }, {"./node": 69}], 47: [function (e, t) {
    var i = e("./node"), n = function (e, t, i) {
  ...
  }, {"./node": 69}], 48: [function (e, t) {
    var i = e("./node"), n = e("../functions/function-caller"), r = function (e, t, i, n) {
    ...
    r.prototype = new i, r.prototype.type = "Call", r.prototype.accept = function (e) {
      ...
      if (o.isValid()) try {
        ...
        throw {type: s.type || "Runtime", message: "error evaluating function `" + this.name + "`" + (s.message ? ": " + s.message : ""), index: this.index, filename: this.currentFileInfo.filename};
      }
  ...
  }, {"../functions/function-caller": 20, "./node": 69}], 49: [function (e, t) {
    function n(e) {
      return "#" + e.map(function (e) {
        return e = Math.min(Math.max(Math.round(e), 0), 255), (16 > e ? "0" : "") + e.toString(16);
      }).join("");
    }
    var r = e("./node"), o = e("../data/colors"), s = function (e, t, i) {
    ...
    s.prototype = new r, s.prototype.type = "Color", s.prototype.luma = function () {
      ...
      return e = 0.03928 >= e ? e / 12.92 : Math.pow((e + 0.055) / 1.055, 2.4), t = 0.03928 >= t ? t / 12.92 : Math.pow((t + 0.055) / 1.055, 2.4), i = 0.03928 >= i ? i / 12.92 : Math.pow((i + 0.055) / 1.055, 2.4), 0.2126 * e + 0.7152 * t + 0.0722 * i;
      ...
      if (r = this.fround(e, this.alpha), 1 > r) return "rgba(" + this.rgb.map(function (e) {
        return Math.min(Math.max(Math.round(e), 0), 255);
      }).concat(Math.min(Math.max(r, 0), 1)).join("," + (o ? "" : " ")) + ")";
      if (n = this.toRGB(), o) {
        var s = n.split("");
      ...
      var e, t, i = this.rgb[0] / 255, n = this.rgb[1] / 255, r = this.rgb[2] / 255, o = this.alpha, s = Math.max(i, n, r), a = Math.min(i, n, r), l = (s + a) / 2, h = s - a;
      if (s === a) e = t = 0; else {
        switch (t = l > 0.5 ? h / (2 - s - a) : h / (s + a), s) {
          case i:
            e = (n - r) / h + (r > n ? 6 : 0);
            break;
          case n:
      ...
      var e, t, i = this.rgb[0] / 255, n = this.rgb[1] / 255, r = this.rgb[2] / 255, o = this.alpha, s = Math.max(i, n, r), a = Math.min(i, n, r), l = s, h = s - a;
      if (t = 0 === s ? 0 : h / s, s === a) e = 0; else {
        switch (s) {
          case i:
            e = (n - r) / h + (r > n ? 6 : 0);
            break;
      ...
      return e.rgb && e.rgb[0] === this.rgb[0] && e.rgb[1] === this.rgb[1] && e.rgb[2] === this.rgb[2] && e.alpha === this.alpha ? 0 : undefined;
    }, s.fromKeyword = function (e) {
      var t, i = e.toLowerCase();
      return o.hasOwnProperty(i) ? t = new s(o[i].slice(1)) : "transparent" === i && (t = new s([0, 0, 0], 0)), t ? (t.value = e, t) : undefined;
    }, t.exports = s;
  }, {"../data/colors": 11, "./node": 69}], 50: [function (e, t) {
    var i = e("./node"), n = function (e) {
      " " === e ? (this.value = " ", this.emptyOrWhitespace = true) : (this.value = e ? e.trim() : "", this.emptyOrWhitespace = "" === this.value);
    };
  ...
  }, {"./node": 69}], 51: [function (e, t) {
    var i = e("./node"), n = e("./debug-info"), r = function (e, t, i, n) {
    ...
    r.prototype = new i, r.prototype.type = "Comment", r.prototype.genCSS = function (e, t) {
      ...
      var t = this.currentFileInfo && this.currentFileInfo.reference && !this.isReferenced, i = e.compress && "!" !== this.value[2];
      return this.isLineComment || t || i;
    }, r.prototype.markReferenced = function () {
      this.isReferenced = true;
    }, t.exports = r;
  }, {"./debug-info": 53, "./node": 69}], 52: [function (e, t) {
    var i = e("./node"), n = function (e, t, i, n, r) {
    ...
    var i = e("./node"), n = e("../contexts"), r = function (e, t) {
      this.ruleset = e, this.frames = t;
    };
    r.prototype = new i, r.prototype.type = "DetachedRuleset", r.prototype.evalFirst = true, r.prototype.accept = function (e) {
  ...
  }, {"../contexts": 10, "./node": 69}], 55: [function (e, t) {
    var i = e("./node"), n = e("../data/unit-conversions"), r = e("./unit"), o = e("./color"), s = function (e, t) {
      this.value = parseFloat(e), this.unit = t && t instanceof r ? t : new r(t ? [t] : undefined);
    };
    s.prototype = new i, s.prototype.type = "Dimension", s.prototype.accept = function (e) {
      ...
      if (0 !== i && 0.000001 > i && i > -0.000001 && (n = i.toFixed(20).replace(/0+$/, "")), e && e.compress) {
      ...
      var t, n;
      if (e instanceof s) {
        if (this.unit.isEmpty() || e.unit.isEmpty()) t = this, n = e; else if (t = this.unify(), n = e.unify(), 0 !== t.unit.compare(n.unit)) return;
        return i.numericCompare(t.value, n.value);
      }
    }, s.prototype.unify = function () {
      return this.convertTo({length: "px", duration: "s", angle: "rad"});
    }, s.prototype.convertTo = function (e) {
      var t, i, r, o, a, l = this.value, h = this.unit.clone(), c = {};
      if ("string" == typeof e) {
  ...
  }, {"../data/unit-conversions": 13, "./color": 49, "./node": 69, "./unit": 78}], 56: [function (e, t) {
    var i = e("./node"), n = e("./selector"), r = e("./ruleset"), o = function (e, t, i, r, o, s, a, l) {
      var h;
      if (this.name = e, this.value = t, i) for (Array.isArray(i) ? this.rules = i : (this.rules = [i], this.rules[0].selectors = new n([], null, null, this.index, o).createEmptySelectors()), h = 0; this.rules.length > h; h++) this.rules[h].allowImports = true;
      this.index = r, this.currentFileInfo = o, this.debugInfo = s, this.isReferenced = a, this.isRooted = l || false;
    };
    o.prototype = new i, o.prototype.type = "Directive", o.prototype.accept = function (e) {
      ...
      return t = e.mediaPath, i = e.mediaBlocks, e.mediaPath = [], e.mediaBlocks = [], n && (n = n.eval(e)), r && (r = [r[0].eval(e)], r[0].root = true), e.mediaPath = t, e.mediaBlocks = i, new o(this.name, n, r, this.index, this.currentFileInfo, this.debugInfo, this.isReferenced, this.isRooted);
    }, o.prototype.variable = function (e) {
      return this.rules ? r.prototype.variable.call(this.rules[0], e) : undefined;
    }, o.prototype.find = function () {
      return this.rules ? r.prototype.find.apply(this.rules[0], arguments) : undefined;
    }, o.prototype.rulesets = function () {
      return this.rules ? r.prototype.rulesets.apply(this.rules[0]) : undefined;
    }, o.prototype.markReferenced = function () {
      var e, t;
      if (this.isReferenced = true, this.rules) for (t = this.rules, e = 0; t.length > e; e++) t[e].markReferenced && t[e].markReferenced();
  ...
  }, {"./node": 69, "./ruleset": 75, "./selector": 76}], 57: [function (e, t) {
    var i = e("./node"), n = e("./paren"), r = e("./combinator"), o = function (e, t, i, n) {
    ...
    o.prototype = new i, o.prototype.type = "Element", o.prototype.accept = function (e) {
      ...
      return t instanceof n && (e.firstSelector = true), t = t.toCSS ? t.toCSS(e) : t, e.firstSelector = i, "" === t && "&" === this.combinator.value.charAt(0) ? "" : this.combinator.toCSS(e) + t;
    }, t.exports = o;
  }, {"./combinator": 50, "./node": 69, "./paren": 71}], 58: [function (e, t) {
    var i = e("./node"), n = e("./paren"), r = e("./comment"), o = function (e) {
    ...
    o.prototype = new i, o.prototype.type = "Expression", o.prototype.accept = function (e) {
      ...
      var t, i = this.parens && !this.parensInOp, r = false;
      return i && e.inParenthesis(), this.value.length > 1 ? t = new o(this.value.map(function (t) {
        return t.eval(e);
      })) : 1 === this.value.length ? (this.value[0].parens && !this.value[0].parensInOp && (r = true), t = this.value[0].eval(e)) : t = this, i && e.outOfParenthesis(), this.parens && this.parensInOp && !e.isMathOn() && !r && (t = new n(t)), t;
    }, o.prototype.genCSS = function (e, t) {
  ...
  }, {"./comment": 51, "./node": 69, "./paren": 71}], 59: [function (e, t) {
    var i = e("./node"), n = function r(e, t, i) {
    ...
    n.next_id = 0, n.prototype = new i, n.prototype.type = "Extend", n.prototype.accept = function (e) {
      ...
      this.selfSelectors = [{elements: n}];
    }, t.exports = n;
  }, {"./node": 69}], 60: [function (e, t) {
    var i = e("./node"), n = e("./media"), r = e("./url"), o = e("./quoted"), s = e("./ruleset"), a = e("./anonymous"), l = function (e, t, i, n, r) {
      if (this.options = i, this.index = n, this.path = e, this.features = t, this.currentFileInfo = r, undefined !== this.options.less || this.options.inline) this.css = !this.options.less || this.options.inline; else {
        var o = this.getPath();
        o && /[#\.\&\?\/]css([\?;].*)?$/.test(o) && (this.css = true);
      }
    };
    l.prototype = new i, l.prototype.type = "Import", l.prototype.accept = function (e) {
      ...
      this.css && undefined === this.path.currentFileInfo.reference && (t.add("@import ", this.currentFileInfo, this.index), this.path.genCSS(e, t), this.features && (t.add(" "), this.features.genCSS(e, t)), t.add(";"));
      ...
      var e = this.path;
      return e instanceof r && (e = e.value), e instanceof o ? e.containsVariables() : true;
      ...
      var t = this.path.eval(e), i = this.currentFileInfo && this.currentFileInfo.rootpath;
      if (!(t instanceof r)) {
        if (i) {
          var n = t.value;
      ...
      if (this.options.inline) {
        var o = new a(this.root, 0, {filename: this.importedFilename, reference: this.path.currentFileInfo && this.path.currentFileInfo.reference}, true, true, false);
  ...
  }, {"./anonymous": 45, "./dimension": 55, "./js-eval-node": 63, "./quoted": 72}], 63: [function (e, t) {
    var i = e("./node"), n = e("./variable"), r = function () {};
    r.prototype = new i, r.prototype.evaluateJavaScript = function (e, t) {
      var i, r = this, o = {};
      if (undefined !== t.javascriptEnabled && !t.javascriptEnabled) throw {message: "You are using JavaScript, which has been disabled.", filename: this.currentFileInfo.filename, index: this.index};
      ...
      try {
        ...
        throw {message: "JavaScript evaluation error: " + s.message + " from `" + e + "`", filename: this.currentFileInfo.filename, index: this.index};
      }
      var a = t.frames[0].variables();
      for (var l in a) a.hasOwnProperty(l) && (o[l.slice(1)] = {value: a[l].value, toJS: function () {
        return this.value.eval(t).toCSS();
      }});
      try {
        i = e.call(o);
      } catch (s) {
        throw {message: "JavaScript evaluation error: '" + s.name + ": " + s.message.replace(/["]/g, "'") + "'", filename: this.currentFileInfo.filename, index: this.index};
      }
      return i;
  ...
  }, {"./node": 69, "./variable": 81}], 64: [function (e, t) {
    var i = e("./node"), n = function (e) {
      this.value = e;
    };
  ...
  }, {"./node": 69}], 65: [function (e, t) {
    var i = e("./ruleset"), n = e("./value"), r = e("./selector"), o = e("./anonymous"), s = e("./expression"), a = e("./directive"), l = function (e, t, o, s) {
      this.index = o, this.currentFileInfo = s;
      var a = new r([], null, null, this.index, this.currentFileInfo).createEmptySelectors();
      this.features = new n(t), this.rules = [new i(a, e)], this.rules[0].allowImports = true;
    };
    l.prototype = new a, l.prototype.type = "Media", l.prototype.isRulesetLike = true, l.prototype.accept = function (e) {
      ...
      var i = false;
      e.strictMath || (i = true, e.strictMath = true);
      try {
        t.features = this.features.eval(e);
      } finally {
        i && (e.strictMath = false);
      }
      ...
      if (e.mediaBlocks.length > 1) {
        ...
        t = new i(n, e.mediaBlocks), t.multiMedia = true;
      }
  ...
  }, {"./anonymous": 45, "./directive": 56, "./expression": 58, "./ruleset": 75, "./selector": 76, "./value": 80}], 66: [function (e, t) {
    var i = e("./node"), n = e("./selector"), r = e("./mixin-definition"), o = e("../functions/default"), s = function (e, t, i, r, o) {
    ...
    s.prototype = new i, s.prototype.type = "MixinCall", s.prototype.accept = function (e) {
      ...
      function t(t, i) {
        var n, r, s;
        for (n = 0; 2 > n; n++) {
          for (k[n] = true, o.value(n), r = 0; i.length > r && k[n]; r++) s = i[r], s.matchCondition && (k[n] = k[n] && s.matchCondition(null, e));
          t.matchCondition && (k[n] = k[n] && t.matchCondition(w, e));
        }
        return k[0] || k[1] ? k[0] != k[1] ? k[1] ? R : P : _ : I;
      }
      var i, n, s, a, l, h, c, u, p, f, d, m, v, g, y, b, w = [], x = [], S = false, C = [], k = [], I = -1, _ = 0, R = 1, P = 2;
      for (c = 0; this.arguments.length > c; c++) if (a = this.arguments[c], l = a.value.eval(e), a.expand && Array.isArray(l.value)) for (l = l.value, u = 0; l.length > u; u++) w.push({value: l[u]}); else w.push({name: a.name, value: l});
      for (b = function (t) {
        return t.matchArgs(null, e);
      }, c = 0; e.frames.length > c; c++) if ((i = e.frames[c].find(this.selector, null, b)).length > 0) {
        for (d = true, u = 0; i.length > u; u++) {
          for (n = i[u].rule, s = i[u].path, f = false, p = 0; e.frames.length > p; p++) if (!(n instanceof r) && n === (e.frames[p].originalRuleset || e.frames[p])) {
            f = true;
            break;
          }
          f || n.matchArgs(w, e) && (m = {mixin: n, group: t(n, s)}, m.group !== I && C.push(m), S = true);
        }
        for (o.reset(), g = [0, 0, 0], u = 0; C.length > u; u++) g[C[u].group]++;
        if (g[_] > 0) v = P; else if (v = R, g[R] + g[P] > 1) throw {type: "Runtime", message: "Ambiguous use of `default()` found when matching for `" + this.format(w) + "`", index: this.index, filename: this.currentFileInfo.filename};
        for (u = 0; C.length > u; u++) if (m = C[u].group, m === _ || m === v) try {
          n = C[u].mixin, n instanceof r || (y = n.originalRuleset || n, n = new r("", [], n.rules, null, false), n.originalRuleset = y), Array.prototype.push.apply(x, n.evalCall(e, w, this.important).rules);
        } catch (L) {
          throw {message: L.message, index: this.index, filename: this.currentFileInfo.filename, stack: L.stack};
        }
        if (S) {
      ...
      throw d ? {type: "Runtime", message: "No matching definition was found for `" + this.format(w) + "`", index: this.index, filename: this.currentFileInfo.filename} : {type: "Name", message: this.selector.toCSS().trim() + " is undefined", index: this.index, filename: this.currentFileInfo.filename};
  ...
  }, {"../functions/default": 19, "./mixin-definition": 67, "./node": 69, "./selector": 76}], 67: [function (e, t) {
    var i = e("./selector"), n = e("./element"), r = e("./ruleset"), o = e("./rule"), s = e("./expression"), a = e("../contexts"), l = function (e, t, r, o, s, a) {
    ...
    l.prototype = new r, l.prototype.type = "MixinDefinition", l.prototype.evalFirst = true, l.prototype.accept = function (e) {
      ...
      var l, h, c, u, p, f, d, m, v = new r(null, null), g = this.params.slice(0), y = 0;
      if (t.frames && t.frames[0] && t.frames[0].functionRegistry && (v.functionRegistry = t.frames[0].functionRegistry.inherit()), t = new a.Eval(t, [v].concat(t.frames)), i) for (i = i.slice(0), y = i.length, c = 0; y > c; c++) if (h = i[c], f = h && h.name) {
        for (d = false, u = 0; g.length > u; u++) if (!n[u] && f === g[u].name) {
          n[u] = h.value.eval(e), v.prependRule(new o(f, h.value.eval(e))), d = true;
          break;
        }
        if (d) {
          i.splice(c, 1), c--;
        ...
        throw {type: "Runtime", message: "Named argument for " + this.name + " " + i[c].name + " not found"};
      }
      for (m = 0, c = 0; g.length > c; c++) if (!n[c]) {
        if (h = i && i[m], f = g[c].name) if (g[c].variadic) {
          ...
          if (p = h && h.value) p = p.eval(e); else {
            if (!g[c].value) throw {type: "Runtime", message: "wrong number of arguments for " + this.name + " (" + y + " for " + this.arity + ")"};
      ...
      var e = this.rules ? this.rules.map(function (e) {
        return e.makeImportant ? e.makeImportant(true) : e;
      }) : this.rules, t = new l(this.name, this.params, e, this.condition, this.variadic, this.frames);
      return t;
    }, l.prototype.eval = function (e) {
      ...
      var n, l, h = [], c = this.frames ? this.frames.concat(e.frames) : e.frames, u = this.evalParams(e, new a.Eval(e, c), t, h);
      ...
      var i, n = e && e.length || 0, r = this.optionalParameters, o = e ? e.reduce(function (e, t) {
        return r.indexOf(t.name) < 0 ? e + 1 : e;
      }, 0) : 0;
      if (this.variadic) {
        if (this.required - 1 > o) return false;
      } else {
        if (this.required > o) return false;
        if (n > this.params.length) return false;
      }
      i = Math.min(o, this.arity);
      for (var s = 0; i > s; s++) if (!this.params[s].name && !this.params[s].variadic && e[s].value.eval(t).toCSS() != this.params[s].value.eval(t).toCSS()) return false;
      return true;
    }, t.exports = l;
  }, {"../contexts": 10, "./element": 57, "./expression": 58, "./rule": 73, "./ruleset": 75, "./selector": 76}], 68: [function (e, t) {
    var i = e("./node"), n = e("./operation"), r = e("./dimension"), o = function (e) {
      this.value = e;
    };
  ...
  }, {"./dimension": 55, "./node": 69, "./operation": 70}], 69: [function (e, t) {
    var i = function () {};
    i.prototype.toCSS = function (e) {
      var t = [];
      return this.genCSS(e, {add: function (e) {
        t.push(e);
      }, isEmpty: function () {
        return 0 === t.length;
      }}), t.join("");
      ...
      if (e.type === t.type) {
        if (e = e.value, t = t.value, !Array.isArray(e)) return e === t ? 0 : undefined;
      ...
      return t > e ? -1 : e === t ? 0 : e > t ? 1 : undefined;
    }, t.exports = i;
  }, {}], 70: [function (e, t) {
    var i = e("./node"), n = e("./color"), r = e("./dimension"), o = function (e, t, i) {
    ...
    o.prototype = new i, o.prototype.type = "Operation", o.prototype.accept = function (e) {
      ...
      var t = this.operands[0].eval(e), i = this.operands[1].eval(e);
      if (e.isMathOn()) {
        if (t instanceof r && i instanceof n && (t = t.toColor()), i instanceof r && t instanceof n && (i = i.toColor()), !t.operate) throw {type: "Operation", message: "Operation on an invalid type"};
  ...
  }, {"./color": 49, "./dimension": 55, "./node": 69}], 71: [function (e, t) {
    var i = e("./node"), n = function (e) {
      this.value = e;
    };
  ...
  }, {"./node": 69}], 72: [function (e, t) {
    var i = e("./node"), n = e("./js-eval-node"), r = e("./variable"), o = function (e, t, i, n, r) {
      this.escaped = null == i ? true : i, this.value = t || "", this.quote = e.charAt(0), this.index = n, this.currentFileInfo = r;
    };
    o.prototype = new n, o.prototype.type = "Quoted", o.prototype.genCSS = function (e, t) {
      ...
      var i = this, n = this.value, a = function (t, n) {
        var s = new r("@" + n, i.index, i.currentFileInfo).eval(e, true);
      ...
      return "Quoted" !== e.type || this.escaped || e.escaped ? e.toCSS && this.toCSS() === e.toCSS() ? 0 : undefined : i.numericCompare(this.value, e.value);
    }, t.exports = o;
  }, {"./js-eval-node": 63, "./node": 69, "./variable": 81}], 73: [function (e, t) {
    function i(e, t) {
      var i, n = "", r = t.length, o = {add: function (e) {
        n += e;
      }};
      for (i = 0; r > i; i++) t[i].eval(e).genCSS(e, o);
      return n;
    }
    var n = e("./node"), r = e("./value"), o = e("./keyword"), s = function (e, t, i, o, s, a, l, h) {
      this.name = e, this.value = t instanceof n ? t : new r([t]), this.important = i ? " " + i.trim() : "", this.merge = o, this.index = s, this.currentFileInfo = a, this.inline = l || false, this.variable = undefined !== h ? h : e.charAt && "@" === e.charAt(0);
    };
    s.prototype = new n, s.prototype.type = "Rule", s.prototype.genCSS = function (e, t) {
      ...
      var t, n = false, r = this.name, a = this.variable;
      "string" != typeof r && (r = 1 === r.length && r[0] instanceof o ? r[0].value : i(e, r), a = false), "font" !== r || e.strictMath || (n = true, e.strictMath = true);
      try {
        if (e.importantScope.push({}), t = this.value.eval(e), !this.variable && "DetachedRuleset" === t.type) throw {message: "Rulesets cannot be evaluated on a property.", index: this.index, filename: this.currentFileInfo.filename};
        var l = this.important, h = e.importantScope.pop();
  ...
  }, {"./keyword": 64, "./node": 69, "./value": 80}], 74: [function (e, t) {
    var i = e("./node"), n = e("./variable"), r = function (e) {
      this.variable = e;
    };
  ...
  }, {"./node": 69, "./variable": 81}], 75: [function (e, t) {
    var i = e("./node"), n = e("./rule"), r = e("./selector"), o = e("./element"), s = e("./paren"), a = e("../contexts"), l = e("../functions/function-registry"), h = e("../functions/default"), c = e("./debug-info"), u = function (e, t, i) {
      this.selectors = e, this.rules = t, this._lookups = {}, this.strictImports = i;
    };
    u.prototype = new i, u.prototype.type = "Ruleset", u.prototype.isRuleset = true, u.prototype.isRulesetLike = true, u.prototype.accept = function (e) {
      this.paths ? e.visitArray(this.paths, true) : this.selectors && (this.selectors = e.visitArray(this.selectors)), this.rules && this.rules.length && (this.rules = e.visitArray(this.rules));
    }, u.prototype.eval = function (e) {
      var t, i, r, o, s = this.selectors, a = false;
      if (s && (i = s.length)) {
        for (t = [], h.error({type: "Syntax", message: "it is currently only allowed in parametric mixin guards,"}), o = 0; i > o; o++) r = s[o].eval(e), t.push(r), r.evaldCondition && (a = true);
        h.reset();
      } else a = true;
      var c, p, f = this.rules ? this.rules.slice(0) : null, d = new u(t, f, this.strictImports);
      ...
      for (o = 0; y > o; o++) "MixinCall" === g[o].type ? (f = g[o].eval(e).filter(function (e) {
        return e instanceof n && e.variable ? !d.variable(e.name) : true;
      ...
      return i.evaldCondition ? !i.condition || i.condition.eval(new a.Eval(t, t.frames)) : false;
      ...
      return this._variables || (this._variables = this.rules ? this.rules.reduce(function (e, t) {
        if (t instanceof n && t.variable === true && (e[t.name] = t), "Import" === t.type && t.root && t.root.variables) {
      ...
      var e, t, i = [], n = this.rules, r = n.length;
      ...
      function i(e) {
        return "boolean" == typeof e.isRulesetLike ? e.isRulesetLike : "function" == typeof e.isRulesetLike ? e.isRulesetLike() : false;
      }
      var n, r, o, s, a, l = [], h = [];
      e.tabLevel = e.tabLevel || 0, this.root || e.tabLevel++;
      var u, p = e.compress ? "" : Array(e.tabLevel + 1).join("  "), f = e.compress ? "" : Array(e.tabLevel).join("  "), d = 0, m = 0;
      ...
      if (h = l.concat(h), !this.root) {
        ...
        var v, g = this.paths, y = g.length;
        for (u = e.compress ? "," : ",\n" + f, n = 0; y > n; n++) if (a = g[n], v = a.length) for (n > 0 && t.add(u), e.firstSelector = true, a[0].genCSS(e, t), e.firstSelector = false, r = 1; v > r; r++) a[r].genCSS(e, t);
        t.add((e.compress ? "{" : " {\n") + p);
      }
      for (n = 0; h.length > n; n++) {
        s = h[n], n + 1 === h.length && (e.lastRule = true);
        var b = e.lastRule;
        i(s) && (e.lastRule = false), s.genCSS ? s.genCSS(e, t) : s.value && t.add(s.value.toString()), e.lastRule = b, e.lastRule ? e.lastRule = false : t.add(e.compress ? "" : "\n" + p);
      }
      ...
      var e, t, i, n;
      if (this.paths) for (e = 0; this.paths.length > e; e++) for (i = this.paths[e], t = 0; i.length > t; t++) if (i[t].getIsReferenced && i[t].getIsReferenced()) return true;
      if (this.selectors) for (e = 0; this.selectors.length > e; e++) if (n = this.selectors[e], n.getIsReferenced && n.getIsReferenced()) return true;
      return false;
      ...
      function n(e, t) {
        var i, n;
        if (0 === e.length) i = new s(e[0]); else {
          var a = [];
      ...
      function l(e, t, i) {
        ...
        var s, p, f, d, m, v, g, y, b, w, x = false;
        for (d = [], m = [[]], s = 0; i.elements.length > s; s++) if (y = i.elements[s], "&" !== y.value) {
          var S = r(y);
          if (null != S) {
            u(d, m);
            var C, k = [], I = [];
          ...
          for (x = true, v = [], u(d, m), p = 0; m.length > p; p++) if (g = m[p], 0 === t.length) g.length > 0 && g[0].elements.push(new o(y.combinator, "", y.index, y.currentFileInfo)), v.push(g); else for (f = 0; t.length > f; f++) {
            var R = h(g, t[f], y, i);
            v.push(R);
          }
          m = v, d = [];
      ...
      function h(e, t, i, n) {
        ...
        if (r = [], e.length > 0 ? (r = e.slice(0), s = r.pop(), a = n.createDerived(s.elements.slice(0))) : a = n.createDerived([]), t.length > 0) {
          var l = i.combinator, h = t[0].elements[0];
      ...
      var p, f, d;
      if (f = [], d = l(f, t, i), !d) if (t.length > 0) for (f = [], p = 0; t.length > p; p++) f.push(t[p].concat(i)); else f = [[i]];
      for (p = 0; f.length > p; p++) e.push(f[p]);
    }, t.exports = u;
  }, {"../contexts": 10, "../functions/default": 19, "../functions/function-registry": 21, "./debug-info": 53, "./element": 57, "./node": 69, "./paren": 71, "./rule": 73, "./selector": 76}], 76: [function (e, t) {
    var i = e("./node"), n = e("./element"), r = function (e, t, i, n, r, o) {
      this.elements = e, this.extendList = t, this.condition = i, this.currentFileInfo = r || {}, this.isReferenced = o, i || (this.evaldCondition = true);
    };
    r.prototype = new i, r.prototype.type = "Selector", r.prototype.accept = function (e) {
      ...
      var e = new n("", "&", this.index, this.currentFileInfo), t = [new r([e], null, null, this.index, this.currentFileInfo)];
      return t[0].mediaEmpty = true, t;
  ...
  }, {"./element": 57, "./node": 69}], 77: [function (e, t) {
    var i = e("./node"), n = function (e) {
      this.value = e;
    };
    n.prototype = new i, n.prototype.type = "UnicodeDescriptor", t.exports = n;
  }, {"./node": 69}], 78: [function (e, t) {
    var i = e("./node"), n = e("../data/unit-conversions"), r = function (e, t, i) {
    ...
    r.prototype = new i, r.prototype.type = "Unit", r.prototype.clone = function () {
      ...
      return this.is(e.toString()) ? 0 : undefined;
      ...
      var t;
      for (t = 0; this.numerator.length > t; t++) this.numerator[t] = e(this.numerator[t], false);
      for (t = 0; this.denominator.length > t; t++) this.denominator[t] = e(this.denominator[t], true);
      ...
      for (e in i) if (i.hasOwnProperty(e)) {
        var n = i[e];
        if (n > 0) for (t = 0; n > t; t++) this.numerator.push(e); else if (0 > n) for (t = 0; -n > t; t++) this.denominator.push(e);
      }
      this.numerator.sort(), this.denominator.sort();
    }, t.exports = r;
  }, {"../data/unit-conversions": 13, "./node": 69}], 79: [function (e, t) {
    var i = e("./node"), n = function (e, t, i, n) {
    ...
    n.prototype = new i, n.prototype.type = "Url", n.prototype.accept = function (e) {
      ...
      if (!this.isEvald && (t = this.currentFileInfo && this.currentFileInfo.rootpath, t && "string" == typeof i.value && e.isPathRelative(i.value) && (i.quote || (t = t.replace(/[\(\)'"\s]/g, function (e) {
        ...
        var r = -1 === i.value.indexOf("?") ? "?" : "&", o = r + e.urlArgs;
        -1 !== i.value.indexOf("#") ? i.value = i.value.replace("#", o + "#") : i.value += o;
      }
      return new n(i, this.index, this.currentFileInfo, true);
    }, t.exports = n;
  }, {"./node": 69}], 80: [function (e, t) {
    var i = e("./node"), n = function (e) {
  ...
  }, {"./node": 69}], 81: [function (e, t) {
    var i = e("./node"), n = function (e, t, i) {
      this.name = e, this.index = t, this.currentFileInfo = i || {};
    };
    n.prototype = new i, n.prototype.type = "Variable", n.prototype.eval = function (e) {
      var t, i = this.name;
      if (0 === i.indexOf("@@") && (i = "@" + new n(i.slice(1), this.index, this.currentFileInfo).eval(e).value), this.evaluating) throw {type: "Name", message: "Recursive variable definition for " + i, filename: this.currentFileInfo.filename, index: this.index};
      if (this.evaluating = true, t = this.find(e.frames, function (t) {
        var n = t.variable(i);
        if (n) {
          if (n.important) {
      ...
      })) return this.evaluating = false, t;
  ...
  }, {"./node": 69}], 82: [function (e, t) {
    t.exports = {getLocation: function (e, t) {
      for (var i = e + 1, n = null, r = -1; --i >= 0 && "\n" !== t.charAt(i);) r++;
      return "number" == typeof e && (n = (t.slice(0, e).match(/\n/g) || "").length), {line: n, column: r};
    }};
  }, {}], 83: [function (e, t) {
    var i = e("../tree"), n = e("./visitor"), r = e("../logger"), o = function () {
    ...
    o.prototype = {run: function (e) {
      ...
      if (!e.root) {
        var t, n, r, o, s = [], a = e.rules, l = a ? a.length : 0;
        for (t = 0; l > t; t++) e.rules[t] instanceof i.Extend && (s.push(a[t]), e.extendOnEveryPath = true);
        var h = e.paths;
        for (t = 0; h.length > t; t++) {
          var c = h[t], u = c[c.length - 1], p = u.extendList;
          for (o = p ? p.slice(0).concat(s) : s, o && (o = o.map(function (e) {
            return e.clone();
          })), n = 0; o.length > n; n++) this.foundExtends = true, r = o[n], r.findSelfSelectors(c), r.ruleset = e, 0 === n && (r.firstExtendOnThisSelectorPath = true), this.allExtendsStack[this.allExtendsStack.length - 1].push(r);
        }
    ...
    s.prototype = {run: function (e) {
      ...
      e.filter(function (e) {
        ...
        t[e.index + " " + i] || (t[e.index + " " + i] = true, r.warn("extend '" + i + "' has no matches"));
      });
    }, doExtendChaining: function (e, t, n) {
      var r, o, s, a, l, h, c, u, p = [], f = this;
      for (n = n || 0, r = 0; e.length > r; r++) for (o = 0; t.length > o; o++) h = e[r], c = t[o], h.parent_ids.indexOf(c.object_id) >= 0 || (l = [c.selfSelectors[0]], s = f.findMatch(h, l), s.length && (h.hasFoundMatches = true, h.selfSelectors.forEach(function (e) {
        a = f.extendSelector(s, l, e), u = new i.Extend(c.selector, c.option, 0), u.selfSelectors = a, a[a.length - 1].extendList = [u], p.push(u), u.ruleset = c.ruleset, u.parent_ids = u.parent_ids.concat(c.parent_ids, h.parent_ids), c.firstExtendOnThisSelectorPath && (u.firstExtendOnThisSelectorPath = true, c.ruleset.paths.push(a));
      })));
      if (p.length) {
        if (this.extendChainCount++, n > 100) {
          var d = "{unable to calculate}", m = "{unable to calculate}";
          try {
            d = p[0].selfSelectors[0].toCSS(), m = p[0].selector.toCSS();
          } catch (v) {}
          throw {message: "extend circular reference detected. One of the circular extends is currently:" + d + ":extend(" + m + ")"};
        }
      ...
      if (!e.root) {
        var t, i, n, r, o = this.allExtendsStack[this.allExtendsStack.length - 1], s = [], a = this;
        for (n = 0; o.length > n; n++) for (i = 0; e.paths.length > i; i++) if (r = e.paths[i], !e.extendOnEveryPath) {
          var l = r[r.length - 1].extendList;
          l && l.length || (t = this.findMatch(o[n], r), t.length && (o[n].hasFoundMatches = true, o[n].selfSelectors.forEach(function (e) {
    ...
    }, visitRulesetOut: function () {}, visitMedia: function (e) {
  ...
  }, {"../logger": 32, "../tree": 61, "./visitor": 89}], 84: [function (e, t) {
    function i(e) {
      this.imports = [], this.variableImports = [], this._onSequencerEmpty = e, this._currentDepth = 0;
    }
    i.prototype.addImport = function (e) {
      var t = this, i = {callback: e, args: null, isReady: false};
      return this.imports.push(i), function () {
        i.args = Array.prototype.slice.call(arguments, 0), i.isReady = true, t.tryRun();
      };
    ...
    var i = e("../contexts"), n = e("./visitor"), r = e("./import-sequencer"), o = function (e, t) {
    ...
    o.prototype = {isReplacing: false, run: function (e) {
      try {
      ...
      if (!e.css || n) {
        var r = new i.Eval(this.context, this.context.frames.slice(0)), o = r.frames[0];
      ...
      try {
        ...
        o.filename || (o.index = e.index, o.filename = e.currentFileInfo.filename), e.css = true, e.error = o;
      }
      if (!n || n.css && !r) this.importCount--, this.isFinished && this._sequencer.tryRun(); else {
        n.options.multiple && (t.importMultiple = true);
        for (var s = undefined === n.css, a = 0; i.rules.length > a; a++) if (i.rules[a] === e) {
          i.rules[a] = n;
          break;
        }
        var l = this.onImported.bind(this, n, t), h = this._sequencer.addImport(l);
      ...
      var s = this, a = e.options.inline, l = e.options.plugin, h = e.options.optional, c = r || o in s.recursionDetector;
      if (t.importMultiple || (e.skip = c ? true : function () {
        return o in s.onceFileDetectionMap ? true : (s.onceFileDetectionMap[o] = true, false);
      }), !o && h && (e.skip = true), n && (e.root = n, e.importedFilename = o, !(a || l || !t.importMultiple && c))) {
        s.recursionDetector[o] = true;
        var u = this.context;
        this.context = t;
        try {
      ...
      "DetachedRuleset" === e.value.type ? this.context.frames.unshift(e) : t.visitDeeper = false;
  ...
  }, {"../contexts": 10, "./import-sequencer": 84, "./visitor": 89}], 86: [function (e, t) {
    var i = {Visitor: e("./visitor"), ImportVisitor: e("./import-visitor"), ExtendVisitor: e("./extend-visitor"), JoinSelectorVisitor: e("./join-selector-visitor"), ToCSSVisitor: e("./to-css-visitor")};
    t.exports = i;
  }, {"./extend-visitor": 83, "./import-visitor": 85, "./join-selector-visitor": 87, "./to-css-visitor": 88, "./visitor": 89}], 87: [function (e, t) {
    var i = e("./visitor"), n = function () {
      this.contexts = [[]], this._visitor = new i(this);
    };
    n.prototype = {run: function (e) {
      ...
      var t, i = this.contexts[this.contexts.length - 1], n = [];
  ...
  }, {"./visitor": 89}], 88: [function (e, t) {
    var i = e("../tree"), n = e("./visitor"), r = function (e) {
      this._visitor = new n(this), this._context = e;
    };
    r.prototype = {isReplacing: true, run: function (e) {
      return this._visitor.visit(e);
    }, visitRule: function (e) {
      return e.variable ? undefined : e;
    }, visitMixinDefinition: function (e) {
      e.frames = [];
    }, visitExtend: function () {}, visitComment: function (e) {
      return e.isSilent(this._context) ? undefined : e;
    }, visitMedia: function (e, t) {
      return e.accept(this._visitor), t.visitDeeper = false, e.rules.length ? e : undefined;
    }, visitImport: function (e) {
      return undefined !== e.path.currentFileInfo.reference && e.css ? undefined : e;
    }, visitDirective: function (e, t) {
      function n(e) {
        var t, i = e.rules;
        1 !== i.length || i[0].paths && 0 !== i[0].paths.length || (i = i[0].rules);
        for (var n = 0; i.length > n; n++) if (t = i[n], t.getIsReferenced && t.getIsReferenced()) return true;
        return false;
      }
      if ("@charset" === e.name) {
      ...
      if (e.rules && e.rules.length) {
        if (this._mergeRules(e.rules[0].rules), e.accept(this._visitor), t.visitDeeper = false, e.getIsReferenced()) return e;
      ...
      for (var t, n = 0; e.length > n; n++) if (t = e[n], t instanceof i.Rule && !t.variable) throw {message: "properties must be inside selector blocks, they cannot be in the root.", index: t.index, filename: t.currentFileInfo ? t.currentFileInfo.filename : null};
    }, visitRuleset: function (e, t) {
      var n, r = [];
      if (e.firstRoot && this.checkPropertiesInRoot(e.rules), e.root) e.accept(this._visitor), t.visitDeeper = false, (e.firstRoot || e.rules && e.rules.length > 0) && r.splice(0, 0, e); else {
        e.paths && (e.paths = e.paths.filter(function (e) {
          var t;
          for (" " === e[0].elements[0].combinator.value && (e[0].elements[0].combinator = new i.Combinator("")), t = 0; e.length > t; t++) if (e[t].getIsReferenced() && e[t].getIsOutput()) return true;
          return false;
        }));
        for (var o = e.rules, s = o ? o.length : 0, a = 0; s > a;) n = o[a], n && n.rules ? (r.push(this._visitor.visit(n)), o.splice(a, 1), s--) : a++;
        s > 0 ? e.accept(this._visitor) : e.rules = null, t.visitDeeper = false, o = e.rules, o && (this._mergeRules(o), o = e.rules), o && (this._removeDuplicateRules(o), o = e.rules), o && o.length > 0 && e.paths.length > 0 && r.splice(0, 0, e);
      }
      ...
      if (e) {
        ...
        for (r = e.length - 1; r >= 0; r--) if (n = e[r], n instanceof i.Rule) if (o[n.name]) {
          ...
          -1 !== t.indexOf(s) ? e.splice(r, 1) : t.push(s);
        } else o[n.name] = n;
      }
      ...
      return e.getIsReferenced() ? (e.accept(this._visitor), e) : undefined;
    }}, t.exports = r;
  }, {"../tree": 61, "./visitor": 89}], 89: [function (e, t) {
    function n(e, t) {
      var i, r;
    ...
    var r = e("../tree"), o = {visitDeeper: true}, s = false, a = function (e) {
      this._implementation = e, this._visitFnCache = [], s || (n(r, 1), s = true);
    };
    a.prototype = {visit: function (e) {
      if (!e) return e;
      var t = e.typeIndex;
      if (!t) return e;
      var n, r = this._visitFnCache, s = this._implementation, a = t << 1, l = 1 | a, h = r[a], c = r[l], u = o;
      if (u.visitDeeper = true, h || (n = "visit" + e.type, h = s[n] || i, c = s[n + "Out"] || i, r[a] = h, r[l] = c), h !== i) {
        var p = h.call(s, e, u);
        s.isReplacing && (e = p);
      }
      ...
      for (i = 0; n > i; i++) {
        ...
        undefined !== o && (o.splice ? o.length && this.flatten(o, r) : r.push(o));
      }
      return r;
    }, flatten: function (e, t) {
      t || (t = []);
      var i, n, r, o, s, a;
      for (n = 0, i = e.length; i > n; n++) if (r = e[n], undefined !== r) if (r.splice) for (s = 0, o = r.length; o > s; s++) a = r[s], undefined !== a && (a.splice ? a.length && this.flatten(a, t) : t.push(a)); else t.push(r);
      return t;
    }}, t.exports = a;
  }, {"../tree": 61}], 90: [function (e, t) {
    function i() {
      if (!s) {
        s = true;
    ...
    function n() {}
    var r = t.exports = {}, o = [], s = false;
    r.nextTick = function (e) {
      o.push(e), s || setTimeout(i, 0);
    }, r.title = "browser", r.browser = true, r.env = {}, r.argv = [], r.version = "", r.versions = {}, r.on = n, r.addListener = n, r.once = n, r.off = n, r.removeListener = n, r.removeAllListeners = n, r.emit = n, r.binding = function () {
    ...
    var n = e("./core.js"), r = e("asap");
    t.exports = n, i.prototype = n.prototype;
    var o = new i(true), s = new i(false), a = new i(null), l = new i(undefined), h = new i(0), c = new i("");
  ...
  }, {"./core.js": 91, asap: 93}], 93: [function (e, t) {
    (function (e) {
      function i() {
        for (; r.next;) {
          r = r.next;
      ...
      function n(t) {
        o = o.next = {task: t, domain: l && e.domain, next: null}, s || (s = true, a());
      }
      var r = {task: undefined, next: null}, o = r, s = false, a = undefined, l = false;
      if ("undefined" != typeof e && e.nextTick) l = true, a = function () {
        e.nextTick(i);
  ...
  function a(e, t, i, n) {
    e.addEventListener ? e[n ? "removeEventListener" : "addEventListener"](t, i, false) : e.attachEvent && e[n ? "detachEvent" : "attachEvent"]("on" + t, i);
  }
  function l(e, c) {
    function d() {
    ...
    function m() {
      ...
      if (d(), M.heat) {
        if (z.length) for (E = z.length; E--;) z[E].el.style[C[z[E].name].heatOn] = j ? k[0 | C[z[E].name].heatmap][Math.round(Math.min((A.fps - 0) / (M.maxFps - 0) * B, B))] : k[0 | C[z[E].name].heatmap][Math.round(Math.min((A.duration - M.threshold) / (0 - M.threshold) * B, B))];
        if (T.graph && C.column.heatOn) for (E = O.length; E--;) O[E].style[C.column.heatOn] = j ? k[0 | C.column.heatmap][Math.round(Math.min((G[E] - 0) / (M.maxFps - 0) * B, B))] : k[0 | C.column.heatmap][Math.round(Math.min(($[E] - M.threshold) / (0 - M.threshold) * B, B))];
      }
    ...
    function w(e) {
      e = e || window.event, e.preventDefault ? (e.preventDefault(), e.stopPropagation()) : (e.returnValue = false, e.cancelBubble = true), A.toggle();
    }
    function x() {
    ...
    function S() {
      if (T.container && x(), C = l.theme[M.theme], k = C.compiledHeatmaps || [], !k.length && C.heatmaps.length) {
        for (F = 0; F < C.heatmaps.length; F++) for (k[F] = [], E = 0; B >= E; E++) {
          var t, n = k[F], r = E;
          t = 0.33 / B * E;
          var o = C.heatmaps[F].saturation, h = C.heatmaps[F].lightness, c = undefined, u = undefined, p = undefined, f = p = undefined, m = c = u = undefined, m = undefined, p = 0.5 >= h ? h * (1 + o) : h + o - h * o;
          0 === p ? t = "#000" : (f = 2 * h - p, u = (p - f) / p, t *= 6, c = Math.floor(t), m = t - c, m *= p * u, 0 === c || 6 === c ? (c = p, u = f + m, p = f) : 1 === c ? (c = p - m, u = p, p = f) : 2 === c ? (c = f, u = p, p = f + m) : 3 === c ? (c = f, u = p - m) : 4 === c ? (c = f + m, u = f) : (c = p, u = f, p -= m), t = "#" + (c = Math.round(255 * c).toString(16), 1 === c.length ? "0" + c : c) + (u = Math.round(255 * u).toString(16), 1 === u.length ? "0" + u : u) + (p = Math.round(255 * p).toString(16), 1 === p.length ? "0" + p : p)), n[r] = t;
        }
      ...
      for (var v in T) T[v] && C[v].heatOn && z.push({name: v, el: T[v]});
    ...
    var C, k, I, _, R, P, L, E, F, A = this, M = o({}, l.defaults, c || {}), T = {}, O = [], B = 100, z = [], D = 0, V = M.threshold, H = 0, N = h() - V, G = [], $ = [], j = "fps" === M.show;
  ...
  l.extend = o, window.FPSMeter = l, l.defaults = {interval: 100, smoothing: 10, show: "fps", toggleOn: "click", decimals: 1, maxFps: 60, threshold: 100, position: "absolute", zIndex: 10, left: "5px", top: "5px", right: "auto", bottom: "auto", margin: "0 0 0 0", theme: "dark", heat: 0, graph: 0, history: 20};
  var g = ["toggleOn", "theme", "heat", "graph", "history"], y = "position zIndex left top right bottom margin".split(" ");
}(window), function (e, t) {
  t.theme = {};
  var i = t.theme.base = {heatmaps: [], container: {heatOn: null, heatmap: null, padding: "5px", minWidth: "95px", height: "30px", lineHeight: "30px", textAlign: "right", textShadow: "none"}, count: {heatOn: null, heatmap: null, position: "absolute", top: 0, right: 0, padding: "5px 10px", height: "30px", fontSize: "24px", fontFamily: "Consolas, Andale Mono, monospace", zIndex: 2}, legend: {heatOn: null, heatmap: null, position: "absolute", top: 0, left: 0, padding: "5px 10px", height: "30px", fontSize: "12px", lineHeight: "32px", fontFamily: "sans-serif", textAlign: "left", zIndex: 2}, graph: {heatOn: null, heatmap: null, position: "relative", boxSizing: "padding-box", MozBoxSizing: "padding-box", height: "100%", zIndex: 1}, column: {width: 4, spacing: 1, heatOn: null, heatmap: null}};
  t.theme.dark = t.extend({}, i, {heatmaps: [{saturation: 0.8, lightness: 0.8}], container: {background: "#222", color: "#fff", border: "1px solid #1a1a1a", textShadow: "1px 1px 0 #222"}, count: {heatOn: "color"}, column: {background: "#3f3f3f"}}), t.theme.light = t.extend({}, i, {heatmaps: [{saturation: 0.5, lightness: 0.5}], container: {color: "#666", background: "#fff", textShadow: "1px 1px 0 rgba(255,255,255,.5), -1px -1px 0 rgba(255,255,255,.5)", boxShadow: "0 0 0 1px rgba(0,0,0,.1)"}, count: {heatOn: "color"}, column: {background: "#eaeaea"}}), t.theme.colorful = t.extend({}, i, {heatmaps: [{saturation: 0.5, lightness: 0.6}], container: {heatOn: "backgroundColor", background: "#888", color: "#fff", textShadow: "1px 1px 0 rgba(0,0,0,.2)", boxShadow: "0 0 0 1px rgba(0,0,0,.1)"}, column: {background: "#777", backgroundColor: "rgba(0,0,0,.2)"}}), t.theme.transparent = t.extend({}, i, {heatmaps: [{saturation: 0.8, lightness: 0.5}], container: {padding: 0, color: "#fff", textShadow: "1px 1px 0 rgba(0,0,0,.5)"}, count: {padding: "0 5px", height: "40px", lineHeight: "40px"}, legend: {padding: "0 5px", height: "40px", lineHeight: "42px"}, graph: {height: "40px"}, column: {width: 5, background: "#999", heatOn: "backgroundColor", opacity: 0.5}});
}(window, FPSMeter), pc.Angle = function (e) {
  this._theta = e / pc.Angle.RA || 0, this._normalize();
}, pc.Angle.RA = 180 / Math.PI, pc.Angle.prototype = {clone: function () {
  var e = new pc.Angle;
...
}}, Object.defineProperty(pc.Angle.prototype, "radians", {get: function () {
  return this._theta;
}, set: function (e) {
  this._theta = e, this._normalize();
}}), Object.defineProperty(pc.Angle.prototype, "degrees", {get: function () {
  return this._theta * pc.Angle.RA;
}, set: function (e) {
  this._theta = e / pc.Angle.RA, this._normalize();
}}), Object.defineProperty(pc.Angle.prototype, "x", {get: function () {
  return Math.sin(this._theta);
}}), Object.defineProperty(pc.Angle.prototype, "y", {get: function () {
...
Garage.attributes.add("garage", {type: "entity"}), Garage.attributes.add("totalStraightRoads", {type: "number", default: 7}), Garage.v1 = new pc.Vec3, Garage.GLOW_TIME = 0.4, Garage.prototype.initialize = function () {
  ...
  if (this.glowTimer > 0) {
    this.glowTimer -= e;
    var t = this.current.model.model.meshInstances[2], i = 2 / Garage.GLOW_TIME, n = 1 - (i * this.glowTimer - 1) * (i * this.glowTimer - 1);
    t.material.setParameter("material_opacity", n), this.glowTimer < 0 && t.material.setParameter("material_opacity", 0.01);
  }
  ...
  this.straightRoads && (this.straightRoads === this.totalStraightRoads && this.spawnGarage(e), this.straightRoads--, 0 === this.straightRoads && (this.app.level.forceStraight = false)), this.blocksUntilStraight--, 0 === this.blocksUntilStraight && (this.blocksUntilStraight = this.getBlocksLeftTillStraight(), this.app.level.forceStraight = true, this.straightRoads = this.totalStraightRoads);
  ...
  t.model.model.meshInstances[1].castShadow = true, t.model.model.meshInstances[1].receiveShadow = true, this.app.root.addChild(t);
  var i = e.getLocalEulerAngles();
  t.setLocalEulerAngles(i);
  var n = Garage.v1;
  n.copy(e.getLocalPosition()), i.y < 80 ? n.x += 4 : n.z -= 4, n.y = 0.02, t.setLocalPosition(n), t.enabled = true, this.garages[e.getGuid()] = t, this.current = t, this.app.fire("garage:spawn", t);
}, Garage.prototype.onFreePoolItem = function (e) {
  e.enabled = false;
  var t = e.getParent();
  t && t.removeChild(e);
}, Garage.prototype.onLevel = function (e) {
  this.current.model.model.meshInstances[2]._hidden = false, this.glowTimer = Garage.GLOW_TIME;
};
var Autoplay = new pc.Script("autoplay");
Autoplay.attributes.add("player", {type: "entity"}), Autoplay.attributes.add("level", {type: "entity"}), Autoplay.prototype.postInitialize = function () {
  return this.app.autoplay ? (this.app.on("app:reset", this.reset, this), void this.reset()) : void (this.enabled = false);
  ...
  var t = this.player.script.player, i = this.level.script.level;
  if (t.block + 1 !== this.currentBlock) {
    var n = i.getBlockType(t.block), r = i.getBlockType(t.block + 1);
    if (n !== r) {
      var o = this.player.getLocalPosition(), s = Math.floor(o.x / 8), a = Math.ceil(o.z / 8);
...
EntityPool.prototype = {expand: function (e) {
  for (var t = this._total; e > t; t++) this._pool.push(this._template.clone()), this._freelist.push(true);
  this._total = e;
}, alloc: function () {
  var e = this._pool[this._nextFree];
  if (this._freelist[this._nextFree] = false, this._allocated++, this._allocated >= this._total) {
  ...
  return -1 === t || this._freelist[t] ? undefined : (this._freelist[t] = true, this._allocated--, this.fire("free", e), true);
}, freeAll: function () {
  for (var e = 0; e < this._total; e++) {
    var t = this._pool[e];
    this._freelist[e] || (this._freelist[e] = true, this.fire("free", t));
  }
  this._allocated = 0;
}};
var StaticGarage = new pc.Script("staticGarage");
StaticGarage.prototype.initialize = function () {
  this.state = true, this.entity.model.model.meshInstances[1].castShadow = true, this.entity.model.model.meshInstances[1].receiveShadow = true, this.app.on("player:current", this.onCurrent, this), this.app.on("app:reset", this.onCurrent, this);
}, StaticGarage.prototype.onCurrent = function (e) {
  e = e || 0, this.state && e > 3 ? (this.state = false, this.entity.enabled = false) : !this.state && 3 > e && (this.state = true, this.entity.enabled = true);
};
var Trail = new pc.Script("trail");
Trail.attributes.add("numQuads", {type: "number", default: "32"}), Trail.attributes.add("quadWidth", {type: "number", default: 0.5}), Trail.attributes.add("quadLength", {type: "number", default: 0.5}), Trail.attributes.add("y", {type: "number", default: 1.153}), Trail.attributes.add("pivot", {type: "entity"}), Trail.attributes.add("texture", {type: "asset", assetType: "texture"}), Trail.attributes.add("shaderVS", {type: "asset", assetType: "shader"}), Trail.attributes.add("shaderPS", {type: "asset", assetType: "shader"}), Trail.prototype.initialize = function () {
  ...
  for (i = 0; i < this.numPoints; i++) n = i, e.push(0), e.push(0.5), e.push(n), e.push(0), e.push(-0.5), e.push(n);
  for (i = 0; i < this.numQuads; i++) t.push(r), t.push(r + 1), t.push(r + 3), t.push(r + 3), t.push(r + 2), t.push(r), r += 2;
  var o = this.app.graphicsDevice, s = pc.scene.procedural.createMesh(o, e, {indices: t}), a = new pc.Material;
  a.cullMode = pc.CULLFACE_NONE, a.alphaWrite = false, a.blend = true, a.blendType = pc.BLEND_NORMAL, a.depthWrite = false, pc.shaderChunks.collectAttribs = function (e) {
    for (var t = {}, i = 0, n = e.indexOf("attribute"); n >= 0;) {
      var r = e.indexOf(";", n), o = e.lastIndexOf(" ", r), s = e.substr(o + 1, r - (o + 1));
  ...
  l.node = this.entity, l.mesh = s, l.material = a, l.cull = false, this.material.setParameter("width", this.quadWidth), this.texture.resource && this.material.setParameter("texture", this.texture.resource), this.texture.on("load", function () {
...
var STATIC_MESHES = true, tempSphere = new pc.BoundingSphere;
pc.ForwardRenderer.prototype._isVisible = function (e, t) {
  return t._hidden ? false : (meshPos = t.aabb.center, t.node._dirtyScale && (t._aabb._radius = t._aabb.halfExtents.length(), t.node._dirtyScale = false), tempSphere.radius = t._aabb._radius, tempSphere.center = meshPos, e._frustum.containsSphere(tempSphere));
};
